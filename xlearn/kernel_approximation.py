...
class PolynomialCountSketch(
    ...
    xlearn.metrics.pairwise.kernel_metrics : List of built-in kernels.
...
class RBFSampler(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from xlearn.kernel_approximation import RBFSampler
    >>> from xlearn.linear_model import SGDClassifier
    >>> X = [[0, 0], [1, 1], [1, 0], [0, 1]]
    >>> y = [0, 0, 1, 1]
...
class SkewedChi2Sampler(
    ...
    RBFSampler : Approximate a RBF kernel feature map using random Fourier
        features.
    SkewedChi2Sampler : Approximate feature map for "skewed chi-squared" kernel.
    xlearn.metrics.pairwise.chi2_kernel : The exact chi squared kernel.
    xlearn.metrics.pairwise.kernel_metrics : List of built-in kernels.
    ...
    Examples
    --------
    >>> from xlearn.kernel_approximation import SkewedChi2Sampler
    >>> from xlearn.linear_model import SGDClassifier
    >>> X = [[0, 0], [1, 1], [1, 0], [0, 1]]
    >>> y = [0, 0, 1, 1]
...
class AdditiveChi2Sampler(TransformerMixin, BaseEstimator):
    ...
    xlearn.metrics.pairwise.additive_chi2_kernel : The exact additive chi
        squared kernel.
    ...
    >>> from xlearn.linear_model import SGDClassifier
    >>> from xlearn.kernel_approximation import AdditiveChi2Sampler
