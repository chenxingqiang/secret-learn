...
from xlearn.decomposition import PCA, TruncatedSVD
...
from xlearn.exceptions import NotFittedError, UnsetMetadataPassedError
...
from xlearn.linear_model import Lasso, LinearRegression, LogisticRegression
from xlearn.metrics import accuracy_score, r2_score
from xlearn.model_selection import train_test_split
from xlearn.neighbors import LocalOutlierFactor
from xlearn.pipeline import FeatureUnion, Pipeline, make_pipeline, make_union
from xlearn.preprocessing import FunctionTransformer, StandardScaler
...
from xlearn.utils import get_tags
from xlearn.utils._metadata_requests import COMPOSITE_METHODS, METHODS
...
from xlearn.utils.validation import _check_feature_names, check_is_fitted
...
def test_xlearn_tags_with_empty_pipeline():
    ...
    expected_tags = be.__xlearn_tags__()
    assert empty_pipeline.__xlearn_tags__() == expected_tags
...
def test_feature_union_check_if_fitted():
    """Check __xlearn_is_fitted__ is defined correctly."""
...
def test_pipeline_warns_not_fitted(method):
    class StatelessEstimator(BaseEstimator):
        ...
        `requires_fit` flag and implement a `__xlearn_check_is_fitted__` returning
        `True`.
        """
