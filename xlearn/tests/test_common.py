...
from xlearn.experimental import (
...
@pytest.mark.filterwarnings(
    "ignore:Importing from xlearn.utils._estimator_html_repr is deprecated."
)
def test_import_all_consistency():
    xlearn_path = [os.path.dirname(xlearn.__file__)]
    # Smoke test to check that any name in a __all__ list is actually defined
    # in the namespace of the module or package.
    pkgs = pkgutil.walk_packages(
        path=xlearn_path, prefix="xlearn.", onerror=lambda _: None
    )
    submods = [modname for _, modname, _ in pkgs]
    for modname in submods + ["xlearn"]:
        if ".tests." in modname or "xlearn.externals" in modname:
            continue
        # Avoid test suite depending on build dependencies, for example Cython
        if "xlearn._build_utils" in modname:
            continue
...
def test_root_import_all_completeness():
    ...
    for _, modname, _ in pkgutil.walk_packages(
        path=xlearn_path, onerror=lambda _: None
    ):
        if "." in modname or modname.startswith("_") or modname in EXCEPTIONS:
            continue
        assert modname in xlearn.__all__
...
def test_all_tests_are_importable():
    ...
    resource_modules = {
        "xlearn.datasets.data",
        "xlearn.datasets.descr",
        "xlearn.datasets.images",
    }
    xlearn_path = [os.path.dirname(xlearn.__file__)]
    lookup = {
        name: ispkg
        for _, name, ispkg in pkgutil.walk_packages(xlearn_path, prefix="xlearn.")
    }
    missing_tests = [
        name
