...
from xlearn.utils._test_common.instance_generator import _construct_instances
from xlearn.utils._testing import (
    _get_func_name,
    check_docstring_parameters,
    ignore_warnings,
)
from xlearn.utils.deprecation import _is_deprecated
...
with warnings.catch_warnings():
    ...
    PUBLIC_MODULES = set(
        [
            pckg[1]
            for pckg in walk_packages(prefix="xlearn.", path=xlearn_path)
            if not any(
                substr in pckg[1] for substr in ["._", ".tests.", "xlearn.externals"]
            )
        ]
    )
...
_DOCSTRING_IGNORES = [
    "xlearn.utils.deprecation.load_mlcomp",
    "xlearn.pipeline.make_pipeline",
    "xlearn.pipeline.make_union",
    "xlearn.utils.extmath.safe_sparse_dot",
    "HalfBinomialLoss",
]
...
def test_fit_docstring_attributes(name, Estimator):
    ...
    if est.__xlearn_tags__().target_tags.one_d_labels:
        est.fit(y)
    elif est.__xlearn_tags__().target_tags.two_d_labels:
        est.fit(np.c_[y, y])
    elif est.__xlearn_tags__().input_tags.three_d_array:
