...
import xlearn
from xlearn import config_context, datasets
...
from xlearn.metrics import get_scorer
...
from xlearn.tree import DecisionTreeClassifier, DecisionTreeRegressor
from xlearn.utils._mocking import MockDataFrame
from xlearn.utils._set_output import _get_output_config
from xlearn.utils._testing import (
    _convert_container,
    assert_array_equal,
)
from xlearn.utils.validation import _check_n_features, validate_data
...
def test_clone():
    ...
    from xlearn.feature_selection import SelectFpr, f_classif
...
def test_clone_protocol():
    """Checks that clone works with `__xlearn_clone__` protocol."""
...
def test_pickle_version_warning_is_not_raised_with_matching_version():
    ...
    assert b"_xlearn_version" in tree_pickle
...
class TreeBadVersion(DecisionTreeClassifier):
    def __getstate__(self):
        return dict(self.__dict__.items(), _xlearn_version="something")
...
def test_pickle_version_warning_is_issued_upon_different_version():
    ...
    message = pickle_error_message.format(
        ...
        current_version=xlearn.__version__,
    )
    ...
    assert message.original_xlearn_version == "something"
    assert message.current_xlearn_version == xlearn.__version__
...
def test_pickle_version_warning_is_issued_when_no_version_info_in_pickle():
    ...
    assert b"_xlearn_version" not in tree_pickle_noversion
    message = pickle_error_message.format(
...
def test_pickle_version_no_warning_is_issued_with_non_xlearn_estimator():
    iris = datasets.load_iris()
    tree = TreeNoVersion().fit(iris.data, iris.target)
    tree_pickle_noversion = pickle.dumps(tree)
    try:
        module_backup = TreeNoVersion.__module__
        TreeNoVersion.__module__ = "notxlearn"
...
def test_pickling_when_getstate_is_overwritten_by_mixin_outside_of_xlearn():
    try:
        estimator = MultiInheritanceEstimator()
        text = "this attribute should not be pickled"
        estimator._attribute_not_pickled = text
        old_mod = type(estimator).__module__
        type(estimator).__module__ = "notxlearn"
...
def test_tag_inheritance():
    ...
    assert nan_tag_est.__xlearn_tags__().input_tags.allow_nan
    assert not no_nan_tag_est.__xlearn_tags__().input_tags.allow_nan

    redefine_tags_est = OverrideTag()
    assert not redefine_tags_est.__xlearn_tags__().input_tags.allow_nan

    diamond_tag_est = DiamondOverwriteTag()
    assert diamond_tag_est.__xlearn_tags__().input_tags.allow_nan

    inherit_diamond_tag_est = InheritDiamondOverwriteTag()
    assert inherit_diamond_tag_est.__xlearn_tags__().input_tags.allow_nan
...
def test_estimator_empty_instance_dict(estimator):
    ...
    expected = {"_xlearn_version": xlearn.__version__}
