...
from xlearn._min_dependencies import PYTEST_MIN_VERSION
...
from xlearn.utils._testing import get_pytest_filterwarning_lines
from xlearn.utils.fixes import (
    _IS_32BIT,
    np_base_version,
    parse_version,
    sp_version,
...
def raccoon_face_or_skip():
    ...
    if scipy_datasets_require_network:
        ...
        if not run_network_tests:
            raise SkipTest("test is enabled when XLEARN_SKIP_NETWORK_TESTS=0")
...
_SKIP32_MARK = pytest.mark.skipif(
    environ.get("XLEARN_RUN_FLOAT32_TESTS", "0") != "1",
    reason="Set XLEARN_RUN_FLOAT32_TESTS=1 to run float32 dtype tests",
)
...
def _fetch_fixture(f):
    ...
    download_if_missing = environ.get("XLEARN_SKIP_NETWORK_TESTS", "1") == "0"
    ...
    def wrapped(*args, **kwargs):
        ...
        try:
            ...
            if str(e) != "Data not found and `download_if_missing` is False":
                raise
            pytest.skip("test is enabled when XLEARN_SKIP_NETWORK_TESTS=0")
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_network = pytest.mark.skip(
        reason="test is enabled when XLEARN_SKIP_NETWORK_TESTS=0"
    )
    ...
    if skip_doctests:
        ...
        for item in items:
            if isinstance(item, DoctestItem):
                ...
                # details.
                if item.name != "xlearn._config.config_context":
    ...
    if not pillow_installed:
        ...
        for item in items:
            if item.name in [
                "xlearn.feature_extraction.image.PatchExtractor",
...
def pytest_generate_tests(metafunc):
    ...
    based on the XLEARN_TESTS_GLOBAL_RANDOM_SEED environment variable.
    ...
    # We rely on XLEARN_TESTS_GLOBAL_RANDOM_SEED environment variable which is
    ...
    random_seed_var = environ.get("XLEARN_TESTS_GLOBAL_RANDOM_SEED")
    ...
    if random_seed_var is None:
        ...
        if min(random_seeds) < 0 or max(random_seeds) > 99:
            raise ValueError(
                ...
                "XLEARN_TESTS_GLOBAL_RANDOM_SEED must be in the range [0, 99] "
...
def pytest_configure(config):
    ...
    if environ.get("XLEARN_WARNINGS_AS_ERRORS", "0") != "0":
    ...
    faulthandler_timeout = int(environ.get("XLEARN_FAULTHANDLER_TIMEOUT", "0"))
