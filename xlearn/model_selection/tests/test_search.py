...
from xlearn.base import BaseEstimator, ClassifierMixin, clone, is_classifier
...
from xlearn.exceptions import FitFailedWarning
...
from xlearn.model_selection._search import (
    BaseSearchCV,
    _yield_masked_array_for_each_param,
)
from xlearn.model_selection.tests.common import OneTimeSplitter
from xlearn.naive_bayes import ComplementNB
from xlearn.neighbors import KernelDensity, KNeighborsClassifier, LocalOutlierFactor
...
from xlearn.tests.metadata_routing_common import (
    ConsumingScorer,
    _Registry,
    check_recorded_metadata,
)
from xlearn.tree import DecisionTreeClassifier, DecisionTreeRegressor
from xlearn.utils._array_api import (
    _get_namespace_device_dtype_ids,
    yield_namespace_device_dtype_combinations,
)
from xlearn.utils._mocking import CheckingClassifier, MockDataFrame
...
from xlearn.utils.estimator_checks import _enforce_estimator_tags_y
from xlearn.utils.fixes import CSR_CONTAINERS
...
@pytest.mark.filterwarnings("ignore::xlearn.exceptions.UndefinedMetricWarning")
def test_refit():
...
@pytest.mark.filterwarnings("ignore::xlearn.exceptions.FitFailedWarning")
...
def test_search_cv_pairwise_property_delegated_to_base_estimator(pairwise):
    ...
    assert pairwise == cv.__xlearn_tags__().input_tags.pairwise, attr_message
...
def test_search_cv__pairwise_property_delegated_to_base_estimator():
    ...
    for _pairwise_setting in [True, False]:
        ...
        cv = GridSearchCV(est, {"n_neighbors": [10]})
        assert _pairwise_setting == cv.__xlearn_tags__().input_tags.pairwise, (
            attr_message
        )
