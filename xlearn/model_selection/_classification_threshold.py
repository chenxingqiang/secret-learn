...
class FixedThresholdClassifier(BaseThresholdClassifier):
    ...
    See Also
    --------
    xlearn.model_selection.TunedThresholdClassifierCV : Classifier that post-tunes
        the decision threshold based on some metrics and using cross-validation.
    xlearn.calibration.CalibratedClassifierCV : Estimator that calibrates
        probabilities.
...
class TunedThresholdClassifierCV(BaseThresholdClassifier):
    ...
    See Also
    --------
    xlearn.model_selection.FixedThresholdClassifier : Classifier that uses a
        constant threshold.
    xlearn.calibration.CalibratedClassifierCV : Estimator that calibrates
        probabilities.
    ...
    >>> from xlearn.model_selection import TunedThresholdClassifierCV, train_test_split
    >>> X, y = make_classification(
    ...     n_samples=1_000, weights=[0.9, 0.1], class_sep=0.8, random_state=42
    ... )
    >>> X_train, X_test, y_train, y_test = train_test_split(
