...
_global_config = {
    "assume_finite": bool(os.environ.get("XLEARN_ASSUME_FINITE", False)),
    "working_memory": int(os.environ.get("XLEARN_WORKING_MEMORY", 1024)),
...
def config_context(
    *,
    ...
    --------
    >>> import xlearn
    >>> from xlearn.utils.validation import assert_all_finite
    >>> with xlearn.config_context(assume_finite=True):
    ...     assert_all_finite([float('nan')])
    >>> with xlearn.config_context(assume_finite=True):
    ...     with xlearn.config_context(assume_finite=False):
    ...         assert_all_finite([float('nan')])
