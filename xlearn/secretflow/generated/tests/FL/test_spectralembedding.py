...
from xlearn.secretflow.generated.fl_spectralembedding import FLSpectralEmbedding


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_spectralembedding_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_spectralembedding_consistency():
    ...
    sklearn_model = SpectralEmbedding()
    sklearn_model.fit(X)
    ...
    sf_model = FLSpectralEmbedding(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing FLSpectralEmbedding...")
