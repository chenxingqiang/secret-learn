...
from xlearn.secretflow.generated.fl_splinetransformer import FLSplineTransformer


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_splinetransformer_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_splinetransformer_consistency():
    ...
    sklearn_model = SplineTransformer()
    sklearn_model.fit(X)
    ...
    sf_model = FLSplineTransformer(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing FLSplineTransformer...")
