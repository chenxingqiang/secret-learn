...
"""
Tests for FLQuantileTransformer
"""
...
from xlearn.secretflow.generated.fl_quantiletransformer import FLQuantileTransformer


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_quantiletransformer_basic():
    ...
    # Test model
    model = FLQuantileTransformer(spu)
    model.fit(fed_X)
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_quantiletransformer_consistency():
    ...
    sklearn_model = QuantileTransformer()
    sklearn_model.fit(X)
    ...
    sf_model = FLQuantileTransformer(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing FLQuantileTransformer...")
