...
from xlearn.secretflow.generated.sl_variancethreshold import FLVarianceThreshold


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_variancethreshold_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_variancethreshold_consistency():
    ...
    sklearn_model = VarianceThreshold()
    sklearn_model.fit(X)
    ...
    sf_model = FLVarianceThreshold(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing FLVarianceThreshold...")
