...
from xlearn.secretflow.generated.sl_kbinsdiscretizer import FLKBinsDiscretizer


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_kbinsdiscretizer_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_kbinsdiscretizer_consistency():
    ...
    sklearn_model = KBinsDiscretizer()
    sklearn_model.fit(X)
    ...
    sf_model = FLKBinsDiscretizer(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing FLKBinsDiscretizer...")
