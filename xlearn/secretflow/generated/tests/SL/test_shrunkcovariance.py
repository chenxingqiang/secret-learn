...
from xlearn.secretflow.generated.sl_shrunkcovariance import FLShrunkCovariance


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_shrunkcovariance_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_shrunkcovariance_consistency():
    ...
    sklearn_model = ShrunkCovariance()
    sklearn_model.fit(X)
    ...
    sf_model = FLShrunkCovariance(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing FLShrunkCovariance...")
