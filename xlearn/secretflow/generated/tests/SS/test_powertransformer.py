...
from xlearn.secretflow.generated.ss_powertransformer import SSPowerTransformer


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_powertransformer_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_powertransformer_consistency():
    ...
    sklearn_model = PowerTransformer()
    sklearn_model.fit(X)
    ...
    sf_model = SSPowerTransformer(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing SSPowerTransformer...")
