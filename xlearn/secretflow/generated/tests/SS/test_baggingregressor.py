...
from xlearn.secretflow.generated.ss_baggingregressor import SSBaggingRegressor


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_baggingregressor_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_baggingregressor_consistency():
    ...
    sklearn_model = BaggingRegressor()
    ...
    sf_model = SSBaggingRegressor(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing SSBaggingRegressor...")
