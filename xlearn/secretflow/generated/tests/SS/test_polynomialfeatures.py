...
"""
Tests for SSPolynomialFeatures
"""
...
from xlearn.secretflow.generated.ss_polynomialfeatures import SSPolynomialFeatures


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_polynomialfeatures_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_polynomialfeatures_consistency():
    ...
    sklearn_model = PolynomialFeatures()
    sklearn_model.fit(X)
    ...
    sf_model = SSPolynomialFeatures(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing SSPolynomialFeatures...")
