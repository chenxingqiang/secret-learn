...
"""
Tests for SSAffinityPropagation
"""
...
from xlearn.secretflow.generated.ss_affinitypropagation import SSAffinityPropagation


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_affinitypropagation_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_affinitypropagation_consistency():
    ...
    sklearn_model = AffinityPropagation()
    sklearn_model.fit(X)
    ...
    sf_model = SSAffinityPropagation(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing SSAffinityPropagation...")
