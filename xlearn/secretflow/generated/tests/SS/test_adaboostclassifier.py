...
"""
Tests for SSAdaBoostClassifier
"""
...
from xlearn.secretflow.generated.ss_adaboostclassifier import SSAdaBoostClassifier


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_adaboostclassifier_basic():
    ...
    # Test model
    model = SSAdaBoostClassifier(spu)
    model.fit(fed_X)
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_adaboostclassifier_consistency():
    ...
    sf_model = SSAdaBoostClassifier(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing SSAdaBoostClassifier...")
