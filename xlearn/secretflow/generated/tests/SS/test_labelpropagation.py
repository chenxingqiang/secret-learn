...
from xlearn.secretflow.generated.ss_labelpropagation import SSLabelPropagation


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_labelpropagation_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_labelpropagation_consistency():
    ...
    sklearn_model = LabelPropagation()
    ...
    sf_model = SSLabelPropagation(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing SSLabelPropagation...")
