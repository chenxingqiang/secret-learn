...
"""
Tests for SSEmpiricalCovariance
"""
...
from xlearn.secretflow.generated.ss_empiricalcovariance import SSEmpiricalCovariance


@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_empiricalcovariance_basic():
...
@pytest.mark.skipif(not SECRETFLOW_AVAILABLE, reason="SecretFlow not available")
def test_empiricalcovariance_consistency():
    ...
    sklearn_model = EmpiricalCovariance()
    sklearn_model.fit(X)
    ...
    sf_model = SSEmpiricalCovariance(spu)
    sf_model.fit(fed_X)
...
if __name__ == "__main__":
    print(f"Testing SSEmpiricalCovariance...")
