...
"""
Standalone Algorithm Migrator for SecretFlow (UPDATED)

Automatically generates SecretFlow adapters for sklearn algorithms with intelligent
algorithm classification and correct template selection.

Key Features:
- Automatic detection of unsupervised vs supervised algorithms
- Intelligent handling of iterative vs non-iterative algorithms
- Correct usage of partial_fit, warm_start, epochs
- Generates proper transform(), fit_transform() methods

Usage:
    python algorithm_migrator_standalone.py --algorithm sklearn.decomposition.PCA --mode ss

Updated: 2025-11-27 - Integrated algorithm classification and template generation
"""
...
try:
    from .algorithm_classifier import classify_algorithm_from_class, classify_algorithm
    from .template_generator import generate_template
    TOOLS_AVAILABLE = True
except ImportError:
    TOOLS_AVAILABLE = False
    print("⚠️  Warning: algorithm_classifier and template_generator not available")
...
class StandaloneAlgorithmMigrator:
    ...
    def __init__(self, output_dir: str = "xlearn/secretflow/generated"):
    ...
    def migrate_algorithm(
        ...
        Automatically detects algorithm characteristics and generates correct code.
        ...
        print(f"🔄 开始迁移: {algorithm_name} (mode={mode})")

        # Use intelligent classification if available
        if TOOLS_AVAILABLE:
            characteristics = classify_algorithm_from_class(sklearn_class)
            print(f"   📊 算法类型: {characteristics['recommended_implementation']}")
            print(f"   📝 Fit签名: {characteristics['fit_signature']}")

            # Generate using intelligent template
            code = generate_template(algorithm_name, module_name, characteristics, mode)
        else:
            # Fallback to old method
            print("   ⚠️  使用传统模板（推荐安装新工具）")
            if mode == "ss":
...
class {class_name}(_ModelBase):
    ...
    This adapter wraps sklearn's {algorithm_name} (or secret-learn if available)
    ...
    **kwargs : dict
    ...
    >>> from xlearn.secretflow.generated.ss_{algorithm_name.lower()} import {class_name}
    >>>
...
'''
        ...
        if has_fit_transform and not has_fit:  # 如果没有单独的 fit
            methods_section += '''
    def fit_transform(
        self,
...
from xlearn.secretflow.generated.{mode}_{algorithm_name.lower()} import {class_name}
...
'''
    ...
    def _generate_sl_adapter(
        self,
        sklearn_class: Type,
        use_xlearn: bool,
        custom_config: Dict[str, Any] = None
    ) -> str:
        """Generate Split Learning adapter"""

        algorithm_name = sklearn_class.__name__
        class_name = f"SL{algorithm_name}"
        module_name = self._get_module_name(sklearn_class)
...
"""
Split Learning adapter for {algorithm_name}
...
'''
    ...
    def batch_migrate(self, algorithms: List[tuple]):
        ...
        print(f"\\n🚀 Starting batch migration of {len(algorithms)} algorithms...\\n")
        ...
        print(f"\\n✅ Batch migration complete: {success_count}/{len(algorithms)} algorithms")
...
Examples:
  ...
  python algorithm_migrator_standalone.py --batch
    ...
    parser.add_argument("--output", type=str,
                       default="xlearn/secretflow/generated",
                       help="Output directory (default: xlearn/secretflow/generated)")
