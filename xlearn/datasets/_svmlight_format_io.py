...
def _dump_svmlight(X, y, f, multilabel, one_based, comment, query_id):
    if comment:
        f.write(
            (
                "# Generated by dump_svmlight_file from secret-learn %s\n" % __version__
            ).encode()
        )
        f.write(
...
def dump_svmlight_file(
    ...
    *,
    ...
    comment : str or bytes, default=None
        ...
        the file as having been dumped by secret-learn. Note that not all
    ...
    Examples
    --------
    >>> from xlearn.datasets import dump_svmlight_file, make_classification
