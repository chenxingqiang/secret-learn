...
from xlearn.datasets import fetch_openml as fetch_openml_orig
from xlearn.datasets._openml import (
    _get_local_path,
    _open_openml_url,
    _retry_with_clean_cache,
)
from xlearn.utils import Bunch
from xlearn.utils._optional_dependencies import check_pandas_support
...
OPENML_TEST_DATA_MODULE = "xlearn.datasets.tests.data.openml"
...
# the mock data in xlearn/datasets/tests/data/openml/ is not always consistent
...
def _monkey_patch_webbased_functions(context, data_id, gzip_response):
    ...
    def _mock_urlopen_download_data(url, has_gzip_header):
        ...
        # gunzip -c xlearn/datasets/tests/data/openml/id_1/api-v1-jd-1.json.gz | grep '"url"  # noqa: E501
        # "https:\/\/www.openml.org\/data\/v1\/download\/1\/anneal.arff"
        # but the mock filename does not contain anneal.arff and is:
        # xlearn/datasets/tests/data/openml/id_1/data-v1-dl-1.arff.gz.
    ...
    # XXX: Global variable
    if test_offline:
        context.setattr(xlearn.datasets._openml, "urlopen", _mock_urlopen)
...
def test_open_openml_url_unlinks_local_path(monkeypatch, tmpdir, write_to_disk):
    ...
    monkeypatch.setattr(xlearn.datasets._openml, "urlopen", _mock_urlopen)
...
def test_fetch_openml_cache(monkeypatch, gzip_response, tmpdir):
    ...
    monkeypatch.setattr(xlearn.datasets._openml, "urlopen", _mock_urlopen_raise)
...
def test_fetch_openml_verify_checksum(monkeypatch, as_frame, tmpdir, parser):
    ...
    mocked_openml_url = xlearn.datasets._openml.urlopen
    ...
    monkeypatch.setattr(xlearn.datasets._openml, "urlopen", swap_file_mock)
...
def test_fetch_openml_with_ignored_feature(monkeypatch, gzip_response, parser):
    ...
    dataset = xlearn.datasets.fetch_openml(
