...
from xlearn.datasets._base import (
    RemoteFileMetadata,
...
from xlearn.datasets.tests.test_common import check_as_frame
...
def test_load_boston_error():
    ...
    with pytest.raises(ImportError, match=msg):
        from xlearn.datasets import load_boston  # noqa: F401

    # other non-existing function should raise the usual import error
    msg = "cannot import name 'non_existing_function' from 'xlearn.datasets'"
    with pytest.raises(ImportError, match=msg):
        from xlearn.datasets import non_existing_function  # noqa: F401
...
def test_fetch_remote_raise_warnings_with_invalid_url(monkeypatch):
    ...
    monkeypatch.setattr("xlearn.datasets._base.urlretrieve", urlretrieve_mock)
...
def test_fetch_file_using_data_home(monkeypatch, tmpdir):
    ...
    monkeypatch.setattr(
        "xlearn.datasets._base.get_data_home", Mock(return_value=data_home)
    )
