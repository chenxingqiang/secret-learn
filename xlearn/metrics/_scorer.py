"""
The :mod:`xlearn.metrics.scorer` submodule implements a flexible
interface for model selection and evaluation using
arbitrary score functions.

A scorer object is a callable that can be passed to
:class:`~xlearn.model_selection.GridSearchCV` or
:func:`xlearn.model_selection.cross_val_score` as the ``scoring``
...
def get_scorer(scoring):
    ...
    :func:`~xlearn.metrics.get_scorer_names` can be used to retrieve the names
    ...
    if isinstance(scoring, str):
        try:
            ...
            raise ValueError(
                ...
                "Use xlearn.metrics.get_scorer_names() "
...
def _check_multimetric_scoring(estimator, scoring):
    ...
    Parameters
    ----------
    estimator : xlearn estimator instance
...
def get_scorer_names():
    ...
    These names can be passed to :func:`~xlearn.metrics.get_scorer` to
...
def check_scoring(estimator=None, scoring=None, *, allow_none=False, raise_exc=True):
    ...
    >>> from xlearn.metrics import check_scoring
    ...
    >>> from xlearn.metrics import make_scorer, accuracy_score, mean_squared_log_error
    >>> X, y = load_iris(return_X_y=True)
    >>> y *= -1
    ...
    if callable(scoring):
        ...
        if (
            ...
            and module.startswith("xlearn.metrics.")
            and not module.startswith("xlearn.metrics._scorer")
            and not module.startswith("xlearn.metrics.tests.")
        ):
            raise ValueError(
