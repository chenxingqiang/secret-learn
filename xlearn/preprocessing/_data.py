...
def scale(X, *, axis=0, with_mean=True, with_std=True, copy=True):
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~xlearn.preprocessing.scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~xlearn.preprocessing.StandardScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
        leaking: `pipe = make_pipeline(StandardScaler(), LogisticRegression())`.

    Examples
    --------
    >>> from xlearn.preprocessing import scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
def minmax_scale(X, feature_range=(0, 1), *, axis=0, copy=True):
    ...
    .. versionadded:: 0.17
       *minmax_scale* function interface
       to :class:`~xlearn.preprocessing.MinMaxScaler`.
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~xlearn.preprocessing.minmax_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~xlearn.preprocessing.MinMaxScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
...
class StandardScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    :class:`~xlearn.decomposition.PCA` : Further removes the linear
...
class MaxAbsScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from xlearn.preprocessing import MaxAbsScaler
    >>> X = [[ 1., -1.,  2.],
    ...      [ 2.,  0.,  0.],
    ...      [ 0.,  1., -1.]]
...
def maxabs_scale(X, *, axis=0, copy=True):
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~xlearn.preprocessing.maxabs_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~xlearn.preprocessing.MaxAbsScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
    ...
    Examples
    --------
    >>> from xlearn.preprocessing import maxabs_scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class RobustScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    xlearn.decomposition.PCA : Further removes the linear correlation across
...
def robust_scale(
    ...
    *,
    ...
    RobustScaler : Performs centering and scaling using the Transformer API
        (e.g. as part of a preprocessing :class:`~xlearn.pipeline.Pipeline`).
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~xlearn.preprocessing.robust_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~xlearn.preprocessing.RobustScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
        leaking: `pipe = make_pipeline(RobustScaler(), LogisticRegression())`.

    Examples
    --------
    >>> from xlearn.preprocessing import robust_scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
def normalize(X, norm="l2", *, axis=1, copy=True, return_norm=False):
    ...
    Examples
    --------
    >>> from xlearn.preprocessing import normalize
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
def binarize(X, *, threshold=0.0, copy=True):
    ...
    Examples
    --------
    >>> from xlearn.preprocessing import binarize
...
class Binarizer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from xlearn.preprocessing import Binarizer
    >>> X = [[ 1., -1.,  2.],
    ...      [ 2.,  0.,  0.],
    ...      [ 0.,  1., -1.]]
...
class KernelCenterer(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    for :class:`~xlearn.decomposition.KernelPCA` for instance.
    ...
    See Also
    --------
    xlearn.kernel_approximation.Nystroem : Approximate a kernel map
    ...
    Examples
    --------
    >>> from xlearn.preprocessing import KernelCenterer
...
def add_dummy_feature(X, value=1.0):
    ...
    Examples
    --------
    >>> from xlearn.preprocessing import add_dummy_feature
...
def quantile_transform(
    ...
    *,
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~xlearn.preprocessing.quantile_transform` unless
        ...
        training set.
        In general, we recommend using
        :class:`~xlearn.preprocessing.QuantileTransformer` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
...
class PowerTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.preprocessing import PowerTransformer
    >>> pt = PowerTransformer()
    >>> data = [[1, 2], [3, 2], [4, 5]]
    >>> print(pt.fit(data))
    PowerTransformer()
...
def power_transform(X, method="yeo-johnson", *, standardize=True, copy=True):
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.preprocessing import power_transform
    >>> data = [[1, 2], [3, 2], [4, 5]]
    ...
    .. warning:: Risk of data leak.
        Do not use :func:`~xlearn.preprocessing.power_transform` unless you
        know what you are doing. A common mistake is to apply it to the entire
        data *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~xlearn.preprocessing.PowerTransformer` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
