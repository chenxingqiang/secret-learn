...
class CalibratedClassifierCV(ClassifierMixin, MetaEstimatorMixin, BaseEstimator):
    ...
    via :func:`~xlearn.model_selection.cross_val_predict`, which are then
    used for calibration. For prediction, the base estimator, trained using all
    the data, is used. This is the prediction method implemented when
    `probabilities=True` for :class:`~xlearn.svm.SVC` and :class:`~xlearn.svm.NuSVC`
    ...
    :class:`~xlearn.frozen.FrozenEstimator`. In this case all provided
    ...
    cv : int, cross-validation generator, or iterable, default=None
        ...
        :class:`~xlearn.model_selection.StratifiedKFold` is used. If ``y`` is
        neither binary nor multiclass, :class:`~xlearn.model_selection.KFold`
        is used.
        ...
        .. versionchanged:: 1.6
            `"prefit"` is deprecated. Use :class:`~xlearn.frozen.FrozenEstimator`
            instead.
    ...
    ensemble : bool, or "auto", default="auto"
        ...
        :class:`~xlearn.frozen.FrozenEstimator`, and `True` otherwise.
        ...
        :func:`~xlearn.model_selection.cross_val_predict`, which are then
        used for calibration. At prediction time, the classifier used is the
        `estimator` trained on all the data.
        Note that this method is also internally implemented  in
        :mod:`xlearn.svm` estimators with the `probabilities=True` parameter.
    ...
    def get_metadata_routing(self):
        ...
        routing : MetadataRouter
            A :class:`~xlearn.utils.metadata_routing.MetadataRouter` encapsulating
            routing information.
        """
        router = (
...
def calibration_curve(
    ...
    *,
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.calibration import calibration_curve
...
class CalibrationDisplay(_BinaryClassifierCurveDisplayMixin):
    ...
    :func:`~xlearn.calibration.CalibrationDisplay.from_estimator` or
    :func:`~xlearn.calibration.CalibrationDisplay.from_predictions`
    ...
    >>> from xlearn.calibration import calibration_curve, CalibrationDisplay
    ...
    def plot(self, *, ax=None, name=None, ref_line=True, **kwargs):
        ...
        **kwargs : dict
        ...
        Returns
        -------
        display : :class:`~xlearn.calibration.CalibrationDisplay`
    ...
    def from_estimator(
        ...
        **kwargs,
        ...
        ----------
        estimator : estimator instance
            Fitted classifier or a fitted :class:`~xlearn.pipeline.Pipeline`
