...
from xlearn.datasets import make_friedman1, make_regression
from xlearn.decomposition import PCA
from xlearn.ensemble import HistGradientBoostingClassifier, RandomForestClassifier
...
def test_allow_nan_tag_comes_from_estimator():
    ...
    assert model.__xlearn_tags__().input_tags.allow_nan is True

    no_nan_est = NoNaNTag()
    model = SelectFromModel(estimator=no_nan_est)
    assert model.__xlearn_tags__().input_tags.allow_nan is False
