...
class BaseDecisionTree(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def _support_missing_values(self, X):
        return (
            not issparse(X)
            and self.__xlearn_tags__().input_tags.allow_nan
    ...
    def cost_complexity_pruning_path(self, X, y, sample_weight=None):
        ...
        Returns
        -------
        ccp_path : :class:`~xlearn.utils.Bunch`
...
class DecisionTreeClassifier(ClassifierMixin, BaseDecisionTree):
    ...
    tree_ : Tree instance
        ...
        ``help(xlearn.tree._tree.Tree)`` for attributes of Tree object and
...
class ExtraTreeClassifier(DecisionTreeClassifier):
    ...
    xlearn.ensemble.ExtraTreesClassifier : An extra-trees classifier.
    xlearn.ensemble.ExtraTreesRegressor : An extra-trees regressor.
    xlearn.ensemble.RandomForestClassifier : A random forest classifier.
    xlearn.ensemble.RandomForestRegressor : A random forest regressor.
    xlearn.ensemble.RandomTreesEmbedding : An ensemble of
    ...
    >>> from xlearn.tree import ExtraTreeClassifier
...
class ExtraTreeRegressor(DecisionTreeRegressor):
    ...
    >>> from xlearn.tree import ExtraTreeRegressor
    >>> X, y = load_diabetes(return_X_y=True)
    >>> X_train, X_test, y_train, y_test = train_test_split(
    ...     X, y, random_state=0)
