...
class SGDClassifier(BaseSGDClassifier):
    ...
    loss : {'hinge', 'log_loss', 'modified_huber', 'squared_hinge',\
        ...
        - The other losses, 'squared_error', 'huber', 'epsilon_insensitive' and
          ...
          :class:`~xlearn.linear_model.SGDRegressor` for a description.
    ...
    See Also
    --------
    xlearn.svm.LinearSVC : Linear support vector classification.
    ...
    def predict_proba(self, X):
        ...
        the classifier with
        :class:`~xlearn.calibration.CalibratedClassifierCV` instead.
...
class SGDRegressor(BaseSGDRegressor):
    ...
    xlearn.svm.SVR : Epsilon-Support Vector Regression.
    TheilSenRegressor : Theil-Sen Estimator robust multivariate regression model.

    Examples
    --------
    >>> import numpy as np
    >>> from xlearn.linear_model import SGDRegressor
    >>> from xlearn.pipeline import make_pipeline
    >>> from xlearn.preprocessing import StandardScaler
...
class SGDOneClassSVM(OutlierMixin, BaseSGD):
    ...
    technique (e.g. `xlearn.kernel_approximation.Nystroem`) to obtain results
    similar to `xlearn.svm.OneClassSVM` which uses a Gaussian kernel by
    default.
    ...
    See Also
    --------
    xlearn.svm.OneClassSVM : Unsupervised Outlier Detection.

    Notes
    -----
    This estimator has a linear complexity in the number of training samples
    and is thus better suited than the `xlearn.svm.OneClassSVM`
