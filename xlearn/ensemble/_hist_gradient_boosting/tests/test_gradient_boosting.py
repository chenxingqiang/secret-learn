...
import xlearn
from xlearn._loss.loss import (
    AbsoluteError,
    HalfBinomialLoss,
    HalfSquaredError,
    PinballLoss,
)
from xlearn.base import BaseEstimator, TransformerMixin, clone, is_regressor
from xlearn.compose import make_column_transformer
from xlearn.datasets import make_classification, make_low_rank_matrix, make_regression
...
from xlearn.ensemble._hist_gradient_boosting.common import G_H_DTYPE
from xlearn.ensemble._hist_gradient_boosting.grower import TreeGrower
from xlearn.ensemble._hist_gradient_boosting.predictor import TreePredictor
from xlearn.exceptions import NotFittedError
from xlearn.metrics import get_scorer, mean_gamma_deviance, mean_poisson_deviance
from xlearn.model_selection import cross_val_score, train_test_split
from xlearn.pipeline import make_pipeline
from xlearn.preprocessing import KBinsDiscretizer, MinMaxScaler, OneHotEncoder
...
from xlearn.utils.fixes import _IS_32BIT
...
def test_max_depth_max_leaf_nodes():
    ...
    # https://github.com/chenxingqiang/secret-learn/issues/16179
...
def test_early_stopping_with_sample_weights(monkeypatch):
    ...
    monkeypatch.setattr(
        xlearn.ensemble._hist_gradient_boosting.gradient_boosting,
        "check_scoring",
        mock_check_scoring,
    )
...
def test_uint8_predict(Est):
    # Non regression test for
    # https://github.com/chenxingqiang/secret-learn/issues/18408
...
def get_different_bitness_node_ndarray(node_ndarray):
    ...
    # xlearn/ensemble/_hist_gradient_boosting/common.pyx)
...
def test_pandas_nullable_dtype():
    # Non regression test for https://github.com/chenxingqiang/secret-learn/issues/28317
    pd = pytest.importorskip("pandas")
