...
class OPTICS(ClusterMixin, BaseEstimator):
    ...
    p : float, default=2
        ...
        :class:`~xlearn.metrics.pairwise_distances`. When p = 1, this is
    ...
    algorithm : {'auto', 'ball_tree', 'kd_tree', 'brute'}, default='auto'
        ...
        - 'ball_tree' will use :class:`~xlearn.neighbors.BallTree`.
        - 'kd_tree' will use :class:`~xlearn.neighbors.KDTree`.
    ...
    leaf_size : int, default=30
        Leaf size passed to :class:`~xlearn.neighbors.BallTree` or
        :class:`~xlearn.neighbors.KDTree`. This can affect the speed of the
...
def compute_optics_graph(
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.cluster import compute_optics_graph
    >>> X = np.array([[1, 2], [2, 5], [3, 6],
    ...               [8, 7], [8, 8], [7, 3]])
...
def cluster_optics_dbscan(*, reachability, core_distances, ordering, eps):
    ...
    ``labels_`` which are close to a :class:`~xlearn.cluster.DBSCAN` with
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.cluster import cluster_optics_dbscan, compute_optics_graph
    >>> X = np.array([[1, 2], [2, 5], [3, 6],
    ...               [8, 7], [8, 8], [7, 3]])
    >>> ordering, core_distances, reachability, predecessor = compute_optics_graph(
    ...     X,
...
def cluster_optics_xi(
    *,
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.cluster import cluster_optics_xi, compute_optics_graph
    >>> X = np.array([[1, 2], [2, 5], [3, 6],
    ...               [8, 7], [8, 8], [7, 3]])
    >>> ordering, core_distances, reachability, predecessor = compute_optics_graph(
    ...     X,
