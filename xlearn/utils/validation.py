    ...
    if has_inf or has_nan_error:
        ...
        if estimator_name and input_name == "X" and has_nan_error:
            ...
            msg_err += (
                ...
                " to consider xlearn.ensemble.HistGradientBoostingClassifier and"
...
def assert_all_finite(
    ...
    *,
    ...
    Examples
    --------
    >>> from xlearn.utils import assert_all_finite
    >>> import numpy as np
...
def as_float_array(
    ...
    Examples
    --------
    >>> from xlearn.utils import as_float_array
    >>> import numpy as np
...
def check_memory(memory):
    ...
    Examples
    --------
    >>> from xlearn.utils.validation import check_memory
...
def check_consistent_length(*arrays):
    ...
    *arrays : list or tuple of input objects.
    ...
    Examples
    --------
    >>> from xlearn.utils.validation import check_consistent_length
    >>> a = [1, 2, 3]
    >>> b = [2, 3, 4]
...
def indexable(*iterables):
    ...
    *iterables : {lists, dataframes, ndarrays, sparse matrices}
    ...
    Examples
    --------
    >>> from xlearn.utils import indexable
...
def check_X_y(
    ...
    *,
    ...
    Examples
    --------
    >>> from xlearn.utils.validation import check_X_y
    >>> X = [[1, 2], [3, 4], [5, 6]]
    >>> y = [1, 2, 3]
    >>> X, y = check_X_y(X, y)
    >>> X
...
def column_or_1d(y, *, dtype=None, warn=False, device=None):
    ...
    Examples
    --------
    >>> from xlearn.utils.validation import column_or_1d
    >>> column_or_1d([1, 1])
    array([1, 1])
    """
...
def has_fit_parameter(estimator, parameter):
    ...
    >>> from xlearn.utils.validation import has_fit_parameter
...
def check_symmetric(array, *, tol=1e-10, raise_warning=True, raise_exception=False):
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.utils.validation import check_symmetric
...
def _is_fitted(estimator, attributes=None, all_or_any=all):
    ...
    if hasattr(estimator, "__xlearn_is_fitted__"):
        return estimator.__xlearn_is_fitted__()
...
def check_is_fitted(estimator, attributes=None, *, msg=None, all_or_any=all):
    ...
    raises a :class:`~xlearn.exceptions.NotFittedError` with the given message.

    If an estimator does not set any attributes with a trailing underscore, it
    can define a ``__xlearn_is_fitted__`` method returning a boolean to
    specify if the estimator is fitted or not. See
    :ref:`sphx_glr_auto_examples_developing_estimators_xlearn_is_fitted.py`
...
def check_scalar(
    ...
    *,
    ...
    Examples
    --------
    >>> from xlearn.utils.validation import check_scalar
...
def _check_psd_eigenvalues(lambdas, enable_warnings=False):
    ...
    Examples
    --------
    >>> from xlearn.utils.validation import _check_psd_eigenvalues
...
def _to_object_array(sequence):
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.utils.validation import _to_object_array
...
def _check_feature_names(estimator, X, *, reset):
    ...
    .. versionchanged:: 1.6
        Moved from :class:`~xlearn.base.BaseEstimator` to
        :mod:`xlearn.utils.validation`.
...
def validate_data(
    ...
    **check_params,
    ...
    y : array-like of shape (n_samples,), default='no_validation'
        ...
        - If `None`, :func:`~xlearn.utils.check_array` is called on `X`. If
          the estimator's `requires_y` tag is True, then an error will be raised.
        - If `'no_validation'`, :func:`~xlearn.utils.check_array` is called
        ...
        - Otherwise, only `y` with `_check_y` or both `X` and `y` are checked with
          either :func:`~xlearn.utils.check_array` or
          :func:`~xlearn.utils.check_X_y` depending on `validate_separately`.
    ...
    validate_separately : False or tuple of dicts, default=False
        ...
        If `False`, call :func:`~xlearn.utils.check_X_y`. Else, it must be a tuple of
        kwargs to be used for calling :func:`~xlearn.utils.check_array` on `X` and `y`
        respectively.
    ...
    skip_check_array : bool, default=False
        ...
        `n_features_in_` are checked. Otherwise, :func:`~xlearn.utils.check_array`
        is called on `X` and `y`.

    **check_params : kwargs
        Parameters passed to :func:`~xlearn.utils.check_array` or
        :func:`~xlearn.utils.check_X_y`. Ignored if validate_separately
        is not False.
