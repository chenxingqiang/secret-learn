...
from xlearn.utils._array_api import _check_array_api_dispatch
from xlearn.utils.fixes import (
    _IS_32BIT,
    VisibleDeprecationWarning,
    _in_unstable_openblas_configuration,
)
from xlearn.utils.multiclass import check_classification_targets
...
def check_skip_network():
    if int(os.environ.get("XLEARN_SKIP_NETWORK_TESTS", 0)):
...
def create_memmap_backed_data(data, mmap_mode="r", return_folder=False):
    ...
    """
    temp_folder = tempfile.mkdtemp(prefix="xlearn_testing_")
...
def assert_docstring_consistency(
    ...
    Examples
    --------
    >>> from xlearn.metrics import (accuracy_score, classification_report,
    ... mean_absolute_error, mean_squared_error, median_absolute_error)
    >>> from xlearn.utils._testing import assert_docstring_consistency
    ... # doctest: +SKIP
...
def assert_run_python_script_without_output(source_code, pattern=".+", timeout=60):
    ...
    """
    fd, source_file = tempfile.mkstemp(suffix="_src_test_xlearn.py")
    os.close(fd)
    try:
        with open(source_file, "wb") as f:
            f.write(source_code.encode("utf-8"))
        cmd = [sys.executable, source_file]
        cwd = op.normpath(op.join(op.dirname(xlearn.__file__), ".."))
        env = os.environ.copy()
        try:
...
def _array_api_for_tests(array_namespace, device):
    ...
    from xlearn.externals.array_api_compat import get_namespace
