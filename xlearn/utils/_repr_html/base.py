...
class _HTMLDocumentationLinkMixin:
    ...
    - `_doc_link_module`: it corresponds to the root module (e.g. `xlearn`). Using this
    ...
    :func:`xlearn.utils.estimator_html_repr` to generate a link to the API
    ...
    _doc_link_module = "xlearn"
    _doc_link_url_param_generator = None

    @property
    def _doc_link_template(self):
        xlearn_version = parse_version(__version__)
        if xlearn_version.dev is None:
            version_url = f"{xlearn_version.major}.{xlearn_version.minor}"
        else:
            version_url = "dev"
        return getattr(
            self,
