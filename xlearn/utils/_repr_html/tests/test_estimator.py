...
from xlearn.cluster import AgglomerativeClustering, Birch
from xlearn.compose import ColumnTransformer, make_column_transformer
from xlearn.datasets import load_iris
from xlearn.decomposition import PCA, TruncatedSVD
from xlearn.ensemble import StackingClassifier, StackingRegressor, VotingClassifier
...
from xlearn.pipeline import FeatureUnion, Pipeline, make_pipeline
from xlearn.preprocessing import FunctionTransformer, OneHotEncoder, StandardScaler
from xlearn.svm import LinearSVC, LinearSVR
from xlearn.tree import DecisionTreeClassifier
from xlearn.utils._repr_html.base import _HTMLDocumentationLinkMixin
from xlearn.utils._repr_html.estimator import (
    _get_css_style,
    _get_visual_block,
    _write_label_html,
    estimator_html_repr,
...
def test_html_documentation_link_mixin_xlearn(mock_version):
    ...
    with patch("xlearn.utils._repr_html.base.__version__", mock_version):
        mixin = _HTMLDocumentationLinkMixin()

        assert mixin._doc_link_module == "xlearn"
        xlearn_version = parse_version(mock_version)
        # we need to parse the version manually to be sure that this test is passing in
        # other branches than `main` (that is "dev").
        if xlearn_version.dev is None:
            version = f"{xlearn_version.major}.{xlearn_version.minor}"
        else:
            version = "dev"
        assert (
