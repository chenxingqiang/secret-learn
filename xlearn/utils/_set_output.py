...
def _get_adapter_from_container(container):
    ...
    See :class:`xlearn.utils._set_output.ContainerAdapterProtocol` for more
    details.
    """
...
def _get_output_config(method, estimator=None):
    ...
    config : dict
        ...
        - "dense": specifies the dense container for `method`. This can be
          `"default"` or `"pandas"`.
    """
    est_xlearn_output_config = getattr(estimator, "_xlearn_output_config", {})
    if method in est_xlearn_output_config:
        dense_config = est_xlearn_output_config[method]
    else:
...
def _auto_wrap_is_configured(estimator):
    ...
    `_SetOutputMixin` sets `_xlearn_auto_wrap_output_keys` to `set()` if auto wrapping
    is manually disabled.
    """
    auto_wrap_output_keys = getattr(estimator, "_xlearn_auto_wrap_output_keys", set())
    return (
...
class _SetOutputMixin:
    ...
    def __init_subclass__(cls, auto_wrap_output_keys=("transform",), **kwargs):
        ...
        if auto_wrap_output_keys is None:
            cls._xlearn_auto_wrap_output_keys = set()
            return
        ...
        for method, key in method_to_key.items():
            if not hasattr(cls, method) or key not in auto_wrap_output_keys:
                continue
            cls._xlearn_auto_wrap_output_keys.add(key)
    ...
    def set_output(self, *, transform=None):
        ...
        if not hasattr(self, "_xlearn_output_config"):
            self._xlearn_output_config = {}

        self._xlearn_output_config["transform"] = transform
        return self
