...
# TODO(1.8): Update when implementing __xlearn_tags__ is required
@pytest.mark.filterwarnings(
    "ignore:.*no attribute '__xlearn_tags__'.*:DeprecationWarning"
)
...
def test_no___xlearn_tags__with_more_tags():
    """Test that calling `get_tags` on a class that defines `_more_tags` but not
    `__xlearn_tags__` raises an error.
    """
...
def test_tags_no_xlearn_tags_concrete_implementation():
    ...
    `BaseEstimator` that implements `__xlearn_tags__`.
    """
    ...
    # `__xlearn_tags__` by calling `super().__xlearn_tags__()`.
    ...
    # 2nd case, the estimator doesn't implement `__xlearn_tags__` at all.
    class MyEstimator2:
        def __init__(self, *, param=1):
            self.param = param
