...
import xlearn
from xlearn._config import config_context
...
from xlearn.exceptions import NotFittedError, PositiveSpectrumWarning
from xlearn.linear_model import ARDRegression

# TODO: add this estimator into the _mocking module in a further refactoring
from xlearn.metrics.tests.test_score_objects import EstimatorWithFit
from xlearn.neighbors import KNeighborsClassifier
from xlearn.random_projection import _sparse_random_matrix
...
from xlearn.utils._mocking import (
    MockDataFrame,
...
from xlearn.utils.estimator_checks import _NotAnArray
...
def test_memmap():
    ...
    with NamedTemporaryFile(prefix="xlearn-test") as tmp:
...
def test_check_array_links_to_imputer_doc_only_for_X(input_name, retype):
    ...
    extended_msg = (
        ...
        " to consider xlearn.ensemble.HistGradientBoostingClassifier and Regressor"
...
def test_suppress_validation():
    ...
    with pytest.raises(ValueError):
        assert_all_finite(X)
    xlearn.set_config(assume_finite=True)
    assert_all_finite(X)
    xlearn.set_config(assume_finite=False)
    with pytest.raises(ValueError):
        assert_all_finite(X)
