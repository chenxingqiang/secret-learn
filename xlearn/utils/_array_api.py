...
_NUMPY_NAMESPACE_NAMES = {"numpy", "xlearn.externals.array_api_compat.numpy"}
...
def _check_array_api_dispatch(array_api_dispatch):
    ...
    if os.environ.get("SCIPY_ARRAY_API") != "1":
        raise RuntimeError(
            ...
            "before importing xlearn or scipy. More details at: "
...
def get_namespace(*arrays, remove_none=True, remove_types=(str,), xp=None):
    ...
    Namespace support is not enabled by default. To enabled it call:
      ...
      xlearn.set_config(array_api_dispatch=True)

    or:

      with xlearn.config_context(array_api_dispatch=True):
          # your code here
...
def _is_xp_namespace(xp, name):
    return xp.__name__ in (
        ...
        f"xlearn.externals.array_api_compat.{name}",
    )
...
def _count_nonzero(X, axis=None, sample_weight=None, xp=None, device=None):
    """A variant of `xlearn.utils.sparsefuncs.count_nonzero` for the Array API.
