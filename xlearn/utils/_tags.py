...
class InputTags:
    ...
    pairwise : bool, default=False
        ...
        `xlearn.utils.metaestimators._safe_split` to slice rows and
        columns.
...
class TargetTags:
    ...
    required : bool
        ...
        for estimators inheriting from `~xlearn.base.RegressorMixin`
        and `~xlearn.base.ClassifierMixin`.
...
def default_tags(estimator) -> Tags:
    ...
    This ignores any ``__xlearn_tags__`` method that the estimator may have.

    If the estimator is a classifier or a regressor, ``target_tags.required``
    will be set to ``True``, otherwise it will be set to ``False``.

    ``transformer_tags`` will be set to :class:`~.xlearn.utils. TransformerTags` if the
    estimator has a ``transform`` or ``fit_transform`` method, otherwise it will be set
    to ``None``.

    ``classifier_tags`` will be set to :class:`~.xlearn.utils.ClassifierTags` if the
    estimator is a classifier, otherwise it will be set to ``None``.
    a classifier, otherwise it will be set to ``None``.

    ``regressor_tags`` will be set to :class:`~.xlearn.utils.RegressorTags` if the
...
def get_tags(estimator) -> Tags:
    ...
    :class:`~xlearn.BaseEstimator` provides the estimator tags machinery.
    ...
    Returns
    -------
    tags : :class:`~.xlearn.utils.Tags`
        The estimator tags.
    """

    try:
        tags = estimator.__xlearn_tags__()
    except AttributeError as exc:
        # TODO(1.8): turn the warning into an error
        if "object has no attribute '__xlearn_tags__'" in str(exc):
            ...
            warnings.warn(
                ...
                "there are no classes that implement `__xlearn_tags__` "
                "in the MRO and/or all classes in the MRO call "
                "`super().__xlearn_tags__()`. Make sure to inherit from "
                "`BaseEstimator` which implements `__xlearn_tags__` (or "
                "alternatively define `__xlearn_tags__` but we don't recommend "
