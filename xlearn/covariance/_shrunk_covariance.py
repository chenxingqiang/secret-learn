...
def shrunk_covariance(emp_cov, shrinkage=0.1):
    ...
    >>> from xlearn.covariance import empirical_covariance, shrunk_covariance
...
def ledoit_wolf_shrinkage(X, assume_centered=False, block_size=1000):
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.covariance import ledoit_wolf_shrinkage
...
def ledoit_wolf(X, *, assume_centered=False, block_size=1000):
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.covariance import empirical_covariance, ledoit_wolf
    >>> real_cov = np.array([[.4, .2], [.2, .8]])
    >>> rng = np.random.RandomState(0)
    >>> X = rng.multivariate_normal(mean=[0, 0], cov=real_cov, size=50)
...
def oas(X, *, assume_centered=False):
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.covariance import oas
    >>> rng = np.random.RandomState(0)
    >>> real_cov = [[.8, .3], [.3, .4]]
    >>> X = rng.multivariate_normal(mean=[0, 0], cov=real_cov, size=500)
