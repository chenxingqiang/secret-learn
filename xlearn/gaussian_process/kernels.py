...
class Hyperparameter(
    ...
    Examples
    --------
    >>> from xlearn.gaussian_process.kernels import ConstantKernel
    >>> from xlearn.datasets import make_friedman2
    >>> from xlearn.gaussian_process import GaussianProcessRegressor
    >>> from xlearn.gaussian_process.kernels import Hyperparameter
...
class Kernel(metaclass=ABCMeta):
    ...
    Examples
    --------
    >>> from xlearn.gaussian_process.kernels import Kernel, RBF
    >>> import numpy as np
...
class CompoundKernel(Kernel):
    ...
    >>> from xlearn.gaussian_process.kernels import CompoundKernel
...
class Sum(KernelOperator):
    ...
    >>> from xlearn.datasets import make_friedman2
    >>> from xlearn.gaussian_process import GaussianProcessRegressor
    >>> from xlearn.gaussian_process.kernels import RBF, Sum, ConstantKernel
...
class Product(KernelOperator):
    ...
    >>> from xlearn.datasets import make_friedman2
    >>> from xlearn.gaussian_process import GaussianProcessRegressor
    >>> from xlearn.gaussian_process.kernels import (RBF, Product,
    ...            ConstantKernel)
    >>> X, y = make_friedman2(n_samples=500, noise=0, random_state=0)
...
class ConstantKernel(StationaryKernelMixin, GenericKernelMixin, Kernel):
    ...
    >>> from xlearn.datasets import make_friedman2
    >>> from xlearn.gaussian_process import GaussianProcessRegressor
    >>> from xlearn.gaussian_process.kernels import RBF, ConstantKernel
    >>> X, y = make_friedman2(n_samples=500, noise=0, random_state=0)
    >>> kernel = RBF() + ConstantKernel(constant_value=2)
    >>> gpr = GaussianProcessRegressor(kernel=kernel, alpha=5,
    ...         random_state=0).fit(X, y)
...
class WhiteKernel(StationaryKernelMixin, GenericKernelMixin, Kernel):
    ...
    >>> from xlearn.datasets import make_friedman2
    >>> from xlearn.gaussian_process import GaussianProcessRegressor
    >>> from xlearn.gaussian_process.kernels import DotProduct, WhiteKernel
    >>> X, y = make_friedman2(n_samples=500, noise=0, random_state=0)
...
class PairwiseKernel(Kernel):
    """Wrapper for kernels in xlearn.metrics.pairwise.

    A thin wrapper around the functionality of the kernels in
    xlearn.metrics.pairwise.
    ...
    >>> from xlearn.datasets import load_iris
    >>> from xlearn.gaussian_process import GaussianProcessClassifier
    >>> from xlearn.gaussian_process.kernels import PairwiseKernel
    >>> X, y = load_iris(return_X_y=True)
