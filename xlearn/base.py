...
def clone(estimator, *, safe=True):
    ...
    .. versionchanged:: 1.3
        Delegates to `estimator.__xlearn_clone__` if the method exists.
    ...
    safe : bool, default=True
        ...
        that are not estimators. Ignored if `estimator.__xlearn_clone__`
        exists.
    ...
    False
    """
    if hasattr(estimator, "__xlearn_clone__") and not inspect.isclass(estimator):
        return estimator.__xlearn_clone__()
    return _clone_parametrized(estimator, safe=safe)


def _clone_parametrized(estimator, *, safe=True):
    """Default implementation of clone. See :func:`xlearn.base.clone` for details."""
    ...
    # _xlearn_output_config is used by `set_output` to configure the output
    # container of an estimator.
    if hasattr(estimator, "_xlearn_output_config"):
        new_object._xlearn_output_config = copy.deepcopy(
            estimator._xlearn_output_config
        )
    return new_object
...
class BaseEstimator(ReprHTMLMixin, _HTMLDocumentationLinkMixin, _MetadataRequester):
    ...
    def set_params(self, **params):
        ...
        (such as :class:`~xlearn.pipeline.Pipeline`). The latter have
    ...
    def __getstate__(self):
        ...
        if type(self).__module__.startswith("xlearn."):
            return dict(state.items(), _xlearn_version=__version__)
        else:
            return state

    def __setstate__(self, state):
        if type(self).__module__.startswith("xlearn."):
            pickle_version = state.pop("_xlearn_version", "pre-0.18")
            if pickle_version != __version__:
                warnings.warn(
                    InconsistentVersionWarning(
                        estimator_name=self.__class__.__name__,
                        current_xlearn_version=__version__,
                        original_xlearn_version=pickle_version,
                    ),
                )
        try:
...
class ClassifierMixin:
    ...
    - `score` method that default to :func:`~xlearn.metrics.accuracy_score`.
...
class RegressorMixin:
    ...
    - `score` method that default to :func:`~xlearn.metrics.r2_score`.
    - enforce that `fit` requires `y` to be passed through the `requires_y` tag,
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.base import BaseEstimator, RegressorMixin
    >>> # Mixin classes should always be on the left-hand side for a correct MRO
    ...
    def score(self, X, y, sample_weight=None):
        ...
        with default value of :func:`~xlearn.metrics.r2_score`.
        This influences the ``score`` method of all the multioutput
        regressors (except for
        :class:`~xlearn.multioutput.MultiOutputRegressor`).
        """
...
class ClusterMixin:
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.base import BaseEstimator, ClusterMixin
...
class BiclusterMixin:
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.base import BaseEstimator, BiclusterMixin
...
class TransformerMixin(_SetOutputMixin):
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.base import BaseEstimator, TransformerMixin
...
class OneToOneFeatureMixin:
    ...
    and output features, such as :class:`~xlearn.preprocessing.StandardScaler`.

    Examples
    --------
    >>> import numpy as np
    >>> from xlearn.base import OneToOneFeatureMixin, BaseEstimator
...
class ClassNamePrefixFeaturesOutMixin:
    ...
    names out, such as :class:`~xlearn.decomposition.PCA`. For example, if
    :class:`~xlearn.decomposition.PCA` outputs 3 features, then the generated feature
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.base import ClassNamePrefixFeaturesOutMixin, BaseEstimator
...
class DensityMixin:
    ...
    Examples
    --------
    >>> from xlearn.base import DensityMixin
...
class OutlierMixin:
    ...
    --------
    >>> import numpy as np
    >>> from xlearn.base import BaseEstimator, OutlierMixin
...
class MetaEstimatorMixin:
    ...
    Examples
    --------
    >>> from xlearn.base import MetaEstimatorMixin
...
def is_classifier(estimator):
    ...
    Examples
    --------
    >>> from xlearn.base import is_classifier
    >>> from xlearn.cluster import KMeans
    >>> from xlearn.svm import SVC, SVR
    >>> classifier = SVC()
    >>> regressor = SVR()
    >>> kmeans = KMeans()
...
def is_regressor(estimator):
    ...
    Examples
    --------
    >>> from xlearn.base import is_regressor
    >>> from xlearn.cluster import KMeans
    >>> from xlearn.svm import SVC, SVR
    >>> classifier = SVC()
    >>> regressor = SVR()
...
def is_clusterer(estimator):
    ...
    Examples
    --------
    >>> from xlearn.base import is_clusterer
    >>> from xlearn.cluster import KMeans
    >>> from xlearn.svm import SVC, SVR
    >>> classifier = SVC()
    >>> regressor = SVR()
