...
:class:`~xlearn.linear_model.LogisticRegression` or
:class:`~xlearn.ensemble.RandomForestClassifier`. And the second group are
meta-estimators, which are estimators that wrap other estimators.
:class:`~xlearn.pipeline.Pipeline` and :class:`~xlearn.model_selection.GridSearchCV`
...
are implemented in :class:`~xlearn.base.BaseEstimator`, which you should inherit from.
...
constant in :class:`~xlearn.linear_model.SGDClassifier`). It should not, however, take
...
is used in algorithms like :class:`~xlearn.model_selection.GridSearchCV`.
...
interactions with `pytest`)::
  ...
  >>> from xlearn.utils.estimator_checks import check_estimator
...
.. topic:: :class:`base.BaseEstimator` and mixins:
    ...
    MRO.
      ...
      >>> from xlearn.utils.validation import validate_data, check_is_fitted
      >>> from xlearn.utils.multiclass import unique_labels
      >>> from xlearn.metrics import euclidean_distances
...
the following estimator::
    ...
    >>> from xlearn.base import BaseEstimator
...
:func:`base.BaseEstimator.__xlearn_clone__` method. `__xlearn_clone__` must return an
instance of the estimator. `__xlearn_clone__` is useful when an estimator needs to hold
on to some state when :func:`base.clone` is called on the estimator. For example,
:class:`~xlearn.frozen.FrozenEstimator` makes use of this.
...
The :mod:`~xlearn.utils.multiclass` module contains useful functions for working with
...
methods. The estimator tags are an instance of :class:`~xlearn.utils.Tags` returned by
the method :meth:`~xlearn.base.BaseEstimator.__xlearn_tags__`. These tags are used
in different places, such as :func:`~base.is_regressor` or the common checks run by
:func:`~xlearn.utils.estimator_checks.check_estimator` and
:func:`~xlearn.utils.estimator_checks.parametrize_with_checks`, where tags determine
...
:class:`~xlearn.utils.Tags` for more information about individual tags.

It is unlikely that the default values for each tag will suit the needs of your specific
estimator. You can change the default values by defining a `__xlearn_tags__()` method
which returns the new values for your estimator's tags. For example::

    class MyMultiOutputEstimator(BaseEstimator):

        def __xlearn_tags__(self):
            tags = super().__xlearn_tags__()
            tags.target_tags.single_output = False
            tags.non_deterministic = True
            return tags

You can create a new subclass of :class:`~xlearn.utils.Tags` if you wish to add new
...
to have a default value. It can be of the form::
    ...
    class MyEstimator(BaseEstimator):
        def __xlearn_tags__(self):
            tags_orig = super().__xlearn_tags__()
            as_dict = {
...
By default :func:`~xlearn.utils.validation.check_is_fitted` checks if there
are any attributes in the instance with a trailing underscore, e.g. `coef_`.
An estimator can change the behavior by implementing a `__xlearn_is_fitted__`
method taking no input and returning a boolean. If this method exists,
:func:`~xlearn.utils.validation.check_is_fitted` simply returns its output.

See :ref:`sphx_glr_auto_examples_developing_estimators_xlearn_is_fitted.py`
for an example on how to use the API.
...
Estimators inheriting from :class:`~xlearn.base.BaseEstimator` display
...
:func:`~xlearn.utils.estimator_html_repr` on an estimator instance.
...
* Unit tests are an exception to the previous rule;
  ...
  A corollary is that, if ``xlearn.foo`` exports a class or function
  that is implemented in ``xlearn.foo.bar.baz``,
  the test should import it from ``xlearn.foo``.
...
.. currentmodule:: xlearn.utils

The module :mod:`xlearn.utils` contains various functions for doing input
...
See :func:`xlearn.utils.check_random_state` in :ref:`developers-utils`.

Here's a simple example of code using some of the above guidelines::

    from xlearn.utils import check_array, check_random_state
...
do use `xlearn.utils._testing.assert_allclose`.
...
`xlearn.utils._testing.assert_allclose`.
