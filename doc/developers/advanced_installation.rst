...
..
   ...
   Probably would need to change a bit xlearn/_min_dependencies.py since Python is not really a package ...
...
#. Install a recent version of Python (|PythonMinVersion| or later) for
   ...
   .. prompt:: bash $
     ...
     conda create -n xlearn-env -c conda-forge python numpy scipy cython meson-python ninja
...
.. note::
    ...
    to avoid surprises when you import `xlearn`. `meson-python` implements
    editable installs by rebuilding `xlearn` when executing `import xlearn`.
    ...
    install` command once, `xlearn` will automatically be rebuilt when
    importing `xlearn`.
