...
7. When applicable, use the validation tools and scripts in the :mod:`xlearn.utils`
...
* Code with a good unit test coverage (at least 80%, better 100%), check with:
  ...
  .. prompt:: bash
    ...
    pytest --cov xlearn path/to/tests
...
.. dropdown:: Guidelines for writing docstrings
  ...
  * You can use `pytest` to test docstrings, e.g. assuming the
    ...
    .. prompt:: bash
      ...
      pytest --doctest-modules xlearn/ensemble/_forest.py -k RandomForestClassifier
...
.. dropdown:: Workflow to improve test coverage
  ...
  1. Run `pytest --cov xlearn /path/to/tests`. The output lists for each file the line
