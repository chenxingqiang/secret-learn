.. currentmodule:: xlearn.feature_selection
...
The classes in the :mod:`xlearn.feature_selection` module can be used
...
best features for a dataset as follows:
  ...
  >>> from xlearn.feature_selection import f_classif
...
.. dropdown:: L1-recovery and compressive sensing
  ...
  (:class:`~xlearn.linear_model.LassoCV` or
  :class:`~xlearn.linear_model.LassoLarsCV`), though this may lead to
  under-penalized models: including a small number of non-relevant variables
  is not detrimental to prediction score. BIC
  (:class:`~xlearn.linear_model.LassoLarsIC`) tends, on the opposite, to set
...
Tree-based estimators (see the :mod:`xlearn.tree` module and forest
of trees in the :mod:`xlearn.ensemble` module) can be used to compute
...
:class:`~xlearn.feature_selection.SequentialFeatureSelector` transformer.
...
.. dropdown:: Details on Sequential Feature Selection
  ...
  SFS differs from :class:`~xlearn.feature_selection.RFE` and
  :class:`~xlearn.feature_selection.SelectFromModel` in that it does not
  ...
  :class:`~xlearn.feature_selection.RFE` would require only a single fit, and
  :class:`~xlearn.feature_selection.SelectFromModel` always just does a single
