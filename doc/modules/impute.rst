...
:func:`~xlearn.metrics.pairwise.nan_euclidean_distances`, is used to find the
...
:class:`~xlearn.pipeline.Pipeline`, be sure to use the
:class:`~xlearn.pipeline.FeatureUnion` or
:class:`~xlearn.compose.ColumnTransformer` to add the indicator features to
...
Now we create a :class:`~xlearn.pipeline.FeatureUnion`. All features will be
...
wrap this in a :class:`~xlearn.pipeline.Pipeline` with a classifier (e.g., a
:class:`~xlearn.tree.DecisionTreeClassifier`) to be able to make predictions.
