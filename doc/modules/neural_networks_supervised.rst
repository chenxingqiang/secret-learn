...
.. currentmodule:: xlearn.neural_network
...
* Multi-layer Perceptron is sensitive to feature scaling, so it
  ...
  You can use :class:`~xlearn.preprocessing.StandardScaler` for standardization.

    >>> from xlearn.preprocessing import StandardScaler  # doctest: +SKIP
  ...
  :class:`~xlearn.preprocessing.StandardScaler` in a
  :class:`~xlearn.pipeline.Pipeline`

* Finding a reasonable regularization parameter :math:`\alpha` is best done
  using :class:`~xlearn.model_selection.GridSearchCV`, usually in the range
  ``10.0 ** -np.arange(1, 7)``.
