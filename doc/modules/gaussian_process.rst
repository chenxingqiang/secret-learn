...
.. currentmodule:: xlearn.gaussian_process
...
:class:`~xlearn.gaussian_process.kernels.WhiteKernel` component into the
...
.. currentmodule:: xlearn.gaussian_process.kernels
...
.. dropdown:: Gaussian Process Kernel API
   ...
   similar interface as :class:`~xlearn.base.BaseEstimator`, providing the
   methods ``get_params()``, ``set_params()``, and ``clone()``. This allows
   setting kernel values also via meta-estimators such as
   :class:`~xlearn.pipeline.Pipeline` or
   :class:`~xlearn.model_selection.GridSearchCV`. Note that due to the nested
   ...
   but with the hyperparameters set to ``theta``. An illustrative example:
      ...
      >>> from xlearn.gaussian_process.kernels import ConstantKernel, RBF
   ...
   All Gaussian process kernels are interoperable with :mod:`xlearn.metrics.pairwise`
   and vice versa: instances of subclasses of :class:`Kernel` can be passed as
   ``metric`` to ``pairwise_kernels`` from :mod:`xlearn.metrics.pairwise`. Moreover,
