...
.. dropdown:: Complexity
  ...
  1. **Nearest neighbor search.**  Isomap uses
     :class:`~xlearn.neighbors.BallTree` for efficient neighbor search.
...
scaling with data size, ``xlearn`` implements some algorithmic
...
.. dropdown:: Barnes-Hut t-SNE
  ...
  * Barnes-Hut only works with dense input data. Sparse data matrices can only be
    ...
    projection for instance using :class:`~xlearn.decomposition.PCA`
