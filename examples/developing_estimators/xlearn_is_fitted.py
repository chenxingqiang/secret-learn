...
speed-up the check, they can implement the `__xlearn_is_fitted__` method as
shown below.

In this example the custom estimator showcases the usage of the
`__xlearn_is_fitted__` method and the `check_is_fitted` utility function
as developer APIs. The `__xlearn_is_fitted__` method checks fitted status
