...
real-world datasets available in :class:`xlearn.datasets`. The goal is to show
...
and displayed using :class:`~xlearn.metrics.RocCurveDisplay`.
...
# :class:`~xlearn.preprocessing.OrdinalEncoder` is often a good strategy for
# tree-based models such as :class:`~xlearn.ensemble.IsolationForest`, whereas
# neighbors-based models such as :class:`~xlearn.neighbors.LocalOutlierFactor`
...
# option is to use a :class:`~xlearn.preprocessing.RobustScaler`.
...
from xlearn.datasets import fetch_kddcup99
...
# :class:`~xlearn.compose.make_column_selector` to find them instead of passing
# a list made by hand.
...
# %%
from xlearn.preprocessing import MinMaxScaler, SplineTransformer, StandardScaler
...
# %%
# On the one hand, :class:`~xlearn.preprocessing.RobustScaler` scales each
...
# deviation. Furthermore, :class:`~xlearn.preprocessing.RobustScaler` does not
# squash marginal outlier values, contrary to
# :class:`~xlearn.preprocessing.StandardScaler`.
#
# On the other hand, :class:`~xlearn.preprocessing.MinMaxScaler` scales each
...
# :class:`~xlearn.preprocessing.StandardScaler` and
# :class:`~xlearn.preprocessing.SplineTransformer`. Please refer to their
