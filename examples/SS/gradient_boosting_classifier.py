...
"""
Usage Example for SSGradientBoostingClassifier

Usage:
    Terminal 1: python gradient_boosting_classifier.py --party bob
    Terminal 2: python gradient_boosting_classifier.py --party alice

This example demonstrates how to use the privacy-preserving GradientBoostingClassifier
...
from secretlearn.SS.ensemble.gradient_boosting_classifier import SSGradientBoostingClassifier


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSGradientBoostingClassifier')
...
def main():
    ...
    print("="*70)
    print(f" SSGradientBoostingClassifier - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSGradientBoostingClassifier...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSGradientBoostingClassifier(spu)
    model.fit(fed_X)
