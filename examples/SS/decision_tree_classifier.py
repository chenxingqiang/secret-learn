...
"""
Usage Example for SSDecisionTreeClassifier

Usage:
    Terminal 1: python decision_tree_classifier.py --party bob
    Terminal 2: python decision_tree_classifier.py --party alice

This example demonstrates how to use the privacy-preserving DecisionTreeClassifier
...
from secretlearn.SS.tree.decision_tree_classifier import SSDecisionTreeClassifier


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSDecisionTreeClassifier')
...
def main():
    ...
    print("="*70)
    print(f" SSDecisionTreeClassifier - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSDecisionTreeClassifier...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSDecisionTreeClassifier(spu)
    model.fit(fed_X)
