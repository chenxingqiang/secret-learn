...
"""
Usage Example for SSLinearDiscriminantAnalysis

Usage:
    Terminal 1: python linear_discriminant_analysis.py --party bob
    Terminal 2: python linear_discriminant_analysis.py --party alice

This example demonstrates how to use the privacy-preserving LinearDiscriminantAnalysis
...
from secretlearn.SS.discriminant_analysis.linear_discriminant_analysis import SSLinearDiscriminantAnalysis


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSLinearDiscriminantAnalysis')
...
def main():
    ...
    print("="*70)
    print(f" SSLinearDiscriminantAnalysis - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSLinearDiscriminantAnalysis...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSLinearDiscriminantAnalysis(spu)
    model.fit(fed_X)
