...
"""
Usage Example for SSPassiveAggressiveClassifier

Usage:
    Terminal 1: python passive_aggressive_classifier.py --party bob
    Terminal 2: python passive_aggressive_classifier.py --party alice

This example demonstrates how to use the privacy-preserving PassiveAggressiveClassifier
...
from secretlearn.SS.linear_models.passive_aggressive_classifier import SSPassiveAggressiveClassifier


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSPassiveAggressiveClassifier')
...
def main():
    ...
    print("="*70)
    print(f" SSPassiveAggressiveClassifier - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSPassiveAggressiveClassifier...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSPassiveAggressiveClassifier(spu)
    model.fit(fed_X)
