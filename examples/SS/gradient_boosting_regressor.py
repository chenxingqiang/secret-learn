...
"""
Usage Example for SSGradientBoostingRegressor

Usage:
    Terminal 1: python gradient_boosting_regressor.py --party bob
    Terminal 2: python gradient_boosting_regressor.py --party alice

This example demonstrates how to use the privacy-preserving GradientBoostingRegressor
...
from secretlearn.SS.ensemble.gradient_boosting_regressor import SSGradientBoostingRegressor


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSGradientBoostingRegressor')
...
def main():
    ...
    print("="*70)
    print(f" SSGradientBoostingRegressor - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSGradientBoostingRegressor...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSGradientBoostingRegressor(spu)
    model.fit(fed_X)
