...
"""
Usage Example for SSRandomForestClassifier

Usage:
    Terminal 1: python random_forest_classifier.py --party bob
    Terminal 2: python random_forest_classifier.py --party alice

This example demonstrates how to use the privacy-preserving RandomForestClassifier
...
from secretlearn.SS.ensemble.random_forest_classifier import SSRandomForestClassifier


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSRandomForestClassifier')
...
def main():
    ...
    print("="*70)
    print(f" SSRandomForestClassifier - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSRandomForestClassifier...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSRandomForestClassifier(spu)
    model.fit(fed_X)
