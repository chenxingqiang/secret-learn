...
"""
Usage Example for SSGaussianProcessClassifier

Usage:
    Terminal 1: python gaussian_process_classifier.py --party bob
    Terminal 2: python gaussian_process_classifier.py --party alice

This example demonstrates how to use the privacy-preserving GaussianProcessClassifier
...
from secretlearn.SS.gaussian_process.gaussian_process_classifier import SSGaussianProcessClassifier


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSGaussianProcessClassifier')
...
def main():
    ...
    print("="*70)
    print(f" SSGaussianProcessClassifier - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSGaussianProcessClassifier...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSGaussianProcessClassifier(spu)
    model.fit(fed_X)
