...
"""
Usage Example for SSGaussianProcessRegressor

Usage:
    Terminal 1: python gaussian_process_regressor.py --party bob
    Terminal 2: python gaussian_process_regressor.py --party alice

This example demonstrates how to use the privacy-preserving GaussianProcessRegressor
...
from secretlearn.SS.gaussian_process.gaussian_process_regressor import SSGaussianProcessRegressor


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSGaussianProcessRegressor')
...
def main():
    ...
    print("="*70)
    print(f" SSGaussianProcessRegressor - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSGaussianProcessRegressor...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSGaussianProcessRegressor(spu)
    model.fit(fed_X)
