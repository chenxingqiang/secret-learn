...
"""
Usage Example for SSRandomForestRegressor

Usage:
    Terminal 1: python random_forest_regressor.py --party bob
    Terminal 2: python random_forest_regressor.py --party alice

This example demonstrates how to use the privacy-preserving RandomForestRegressor
...
from secretlearn.SS.ensemble.random_forest_regressor import SSRandomForestRegressor


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSRandomForestRegressor')
...
def main():
    ...
    print("="*70)
    print(f" SSRandomForestRegressor - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSRandomForestRegressor...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSRandomForestRegressor(spu)
    model.fit(fed_X)
