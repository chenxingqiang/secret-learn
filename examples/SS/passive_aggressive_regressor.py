...
"""
Usage Example for SSPassiveAggressiveRegressor

Usage:
    Terminal 1: python passive_aggressive_regressor.py --party bob
    Terminal 2: python passive_aggressive_regressor.py --party alice

This example demonstrates how to use the privacy-preserving PassiveAggressiveRegressor
...
from secretlearn.SS.linear_models.passive_aggressive_regressor import SSPassiveAggressiveRegressor


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSPassiveAggressiveRegressor')
...
def main():
    ...
    print("="*70)
    print(f" SSPassiveAggressiveRegressor - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSPassiveAggressiveRegressor...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSPassiveAggressiveRegressor(spu)
    model.fit(fed_X)
