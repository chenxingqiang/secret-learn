...
"""
Usage Example for SSRadiusNeighborsClassifier

Usage:
    Terminal 1: python radius_neighbors_classifier.py --party bob
    Terminal 2: python radius_neighbors_classifier.py --party alice

This example demonstrates how to use the privacy-preserving RadiusNeighborsClassifier
...
from secretlearn.SS.neighbors.radius_neighbors_classifier import SSRadiusNeighborsClassifier


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSRadiusNeighborsClassifier')
...
def main():
    ...
    print("="*70)
    print(f" SSRadiusNeighborsClassifier - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSRadiusNeighborsClassifier...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSRadiusNeighborsClassifier(spu)
    model.fit(fed_X)
