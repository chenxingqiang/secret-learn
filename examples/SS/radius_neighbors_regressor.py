...
"""
Usage Example for SSRadiusNeighborsRegressor

Usage:
    Terminal 1: python radius_neighbors_regressor.py --party bob
    Terminal 2: python radius_neighbors_regressor.py --party alice

This example demonstrates how to use the privacy-preserving RadiusNeighborsRegressor
...
from secretlearn.SS.neighbors.radius_neighbors_regressor import SSRadiusNeighborsRegressor


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSRadiusNeighborsRegressor')
...
def main():
    ...
    print("="*70)
    print(f" SSRadiusNeighborsRegressor - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSRadiusNeighborsRegressor...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSRadiusNeighborsRegressor(spu)
    model.fit(fed_X)
