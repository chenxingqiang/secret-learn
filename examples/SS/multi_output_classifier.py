...
"""
Usage Example for SSMultiOutputClassifier

Usage:
    Terminal 1: python multi_output_classifier.py --party bob
    Terminal 2: python multi_output_classifier.py --party alice

This example demonstrates how to use the privacy-preserving MultiOutputClassifier
...
from secretlearn.SS.multioutput.multi_output_classifier import SSMultiOutputClassifier


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSMultiOutputClassifier')
...
def main():
    ...
    print("="*70)
    print(f" SSMultiOutputClassifier - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSMultiOutputClassifier...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSMultiOutputClassifier(spu)
    model.fit(fed_X)
