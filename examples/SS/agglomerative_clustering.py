...
"""
Usage Example for SSAgglomerativeClustering

Usage:
    Terminal 1: python agglomerative_clustering.py --party bob
    Terminal 2: python agglomerative_clustering.py --party alice

This example demonstrates how to use the privacy-preserving AgglomerativeClustering
...
from secretlearn.SS.clustering.agglomerative_clustering import SSAgglomerativeClustering


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSAgglomerativeClustering')
...
def main():
    ...
    print("="*70)
    print(f" SSAgglomerativeClustering - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSAgglomerativeClustering...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSAgglomerativeClustering(spu)
    model.fit(fed_X)
