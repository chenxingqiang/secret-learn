...
"""
Usage Example for SSDecisionTreeRegressor

Usage:
    Terminal 1: python decision_tree_regressor.py --party bob
    Terminal 2: python decision_tree_regressor.py --party alice

This example demonstrates how to use the privacy-preserving DecisionTreeRegressor
...
from secretlearn.SS.tree.decision_tree_regressor import SSDecisionTreeRegressor


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSDecisionTreeRegressor')
...
def main():
    ...
    print("="*70)
    print(f" SSDecisionTreeRegressor - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSDecisionTreeRegressor...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSDecisionTreeRegressor(spu)
    model.fit(fed_X)
