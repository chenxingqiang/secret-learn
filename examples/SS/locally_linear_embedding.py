...
"""
Usage Example for SSLocallyLinearEmbedding

Usage:
    Terminal 1: python locally_linear_embedding.py --party bob
    Terminal 2: python locally_linear_embedding.py --party alice

This example demonstrates how to use the privacy-preserving LocallyLinearEmbedding
...
from secretlearn.SS.manifold.locally_linear_embedding import SSLocallyLinearEmbedding


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSLocallyLinearEmbedding')
...
def main():
    ...
    print("="*70)
    print(f" SSLocallyLinearEmbedding - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSLocallyLinearEmbedding...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSLocallyLinearEmbedding(spu)
    model.fit(fed_X)
