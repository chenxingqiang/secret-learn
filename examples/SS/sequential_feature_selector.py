...
"""
Usage Example for SSSequentialFeatureSelector

Usage:
    Terminal 1: python sequential_feature_selector.py --party bob
    Terminal 2: python sequential_feature_selector.py --party alice

This example demonstrates how to use the privacy-preserving SequentialFeatureSelector
...
from secretlearn.SS.feature_selection.sequential_feature_selector import SSSequentialFeatureSelector


def parse_args():
    parser = argparse.ArgumentParser(description='SS SSSequentialFeatureSelector')
...
def main():
    ...
    print("="*70)
    print(f" SSSequentialFeatureSelector - Party: {party_name.upper()}")
    print("="*70)
    ...
    # Step 4: Train model
    print("\n[4/5] Training SSSequentialFeatureSelector...")
    print("  Note: All computation happens in SPU's encrypted environment")
    
    import time
    start_time = time.time()
    
    model = SSSequentialFeatureSelector(spu)
    model.fit(fed_X)
