...
:mod:`xlearn.multiclass` includes OvO/OvR strategies used to train a
multiclass classifier by fitting a set of binary classifiers (the
:class:`~xlearn.multiclass.OneVsOneClassifier` and
:class:`~xlearn.multiclass.OneVsRestClassifier` meta-estimators). This example
will review them.
"""
...
# dataset. We use the :func:`xlearn.datasets.fetch_openml` function to load
...
# :class:`~xlearn.tree.DecisionTreeClassifier` and a
# :class:`~xlearn.model_selection.RepeatedStratifiedKFold` cross-validation
# with 3 splits and 5 repetitions.
#
# We compare the following strategies:
#
# * :class:`~xlearn.tree.DecisionTreeClassifier` can handle multiclass
...
# * :class:`~xlearn.multiclass.OneVsOneClassifier` trains a set of binary
#   classifiers where each classifier is trained to distinguish between
#   two classes.
# * :class:`~xlearn.multiclass.OneVsRestClassifier`: trains a set of binary
#   classifiers where each classifier is trained to distinguish between
...
from xlearn.model_selection import RepeatedStratifiedKFold, cross_validate
