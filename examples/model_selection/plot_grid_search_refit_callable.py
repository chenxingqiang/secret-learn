...
minimising the number of :class:`~xlearn.decomposition.PCA` components [1]. It uses
:class:`~xlearn.model_selection.GridSearchCV` with a custom refit callable to select
the optimal model.
...
from xlearn.model_selection import GridSearchCV, ShuffleSplit
...
# callable with :class:`~xlearn.model_selection.GridSearchCV`.
#
# Key takeaways:
# 1. The one-standard-error rule provides a good rule of thumb to select simpler models
# 2. Custom refit callables in :class:`~xlearn.model_selection.GridSearchCV` allow for
