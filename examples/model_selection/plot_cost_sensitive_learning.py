...
We use the :class:`~xlearn.model_selection.TunedThresholdClassifierCV` to select the
...
# :class:`~xlearn.model_selection.TunedThresholdClassifierCV` in a setting of
...
xlearn.set_config(transform_output="pandas")
...
from xlearn.metrics import make_scorer, precision_score, recall_score
...
# ^^^^^^^^^^^^^^^^^^^^^^^^
#
# We use :class:`~xlearn.ensemble.HistGradientBoostingClassifier` as a predictive model
...
from xlearn.metrics import PrecisionRecallDisplay, RocCurveDisplay
...
# :class:`~xlearn.model_selection.TunedThresholdClassifierCV` class is here to help us.
...
# ^^^^^^^^^^^^^^^^^^^^^^^^
#
# We use :class:`~xlearn.model_selection.TunedThresholdClassifierCV` to tune the
...
from xlearn.model_selection import TunedThresholdClassifierCV
...
# :class:`~xlearn.model_selection.TunedThresholdClassifierCV`. In particular, the
...
# :class:`~xlearn.model_selection.TunedThresholdClassifierCV` is not the same as the
...
# :class:`~xlearn.model_selection.TunedThresholdClassifierCV` to optimize the decision
...
# :class:`~xlearn.model_selection.TunedThresholdClassifierCV` is in charge of
...
# :class:`~xlearn.model_selection.TunedThresholdClassifierCV` to find the optimal
...
#
# The class :class:`~xlearn.model_selection.FixedThresholdClassifier` allows us to
...
# we use :class:`~xlearn.frozen.FrozenEstimator` to wrap the predictive model to
# avoid any refitting.
#
...
from xlearn.frozen import FrozenEstimator
from xlearn.model_selection import FixedThresholdClassifier
