...
"""
Secret Sharing adapter for LocalOutlierFactor

LocalOutlierFactor is a SUPERVISED algorithm.
...
class SSLocalOutlierFactor:
    """Secret Sharing LocalOutlierFactor (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSLocalOutlierFactor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSLocalOutlierFactor training in SPU")
