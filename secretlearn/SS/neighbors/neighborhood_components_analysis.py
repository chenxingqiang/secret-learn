...
"""
Secret Sharing adapter for NeighborhoodComponentsAnalysis
...
class SSNeighborhoodComponentsAnalysis:
    """Secret Sharing NeighborhoodComponentsAnalysis (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSNeighborhoodComponentsAnalysis with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSNeighborhoodComponentsAnalysis training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = NeighborhoodComponentsAnalysis(**kwargs)
            model.fit(X, y)
            return model
