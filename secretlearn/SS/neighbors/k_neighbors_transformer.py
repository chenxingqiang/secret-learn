...
"""
Secret Sharing adapter for KNeighborsTransformer
...
class SSKNeighborsTransformer:
    """Secret Sharing KNeighborsTransformer (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSKNeighborsTransformer with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSKNeighborsTransformer training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = KNeighborsTransformer(**kwargs)
            model.fit(X, y)
            return model
