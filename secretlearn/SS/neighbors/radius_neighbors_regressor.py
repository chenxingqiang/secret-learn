...
"""
Secret Sharing adapter for RadiusNeighborsRegressor

RadiusNeighborsRegressor is a SUPERVISED algorithm.
...
try:
    from xlearn.neighbors import RadiusNeighborsRegressor
    USING_XLEARN = True
except ImportError:
...
class SSRadiusNeighborsRegressor:
    """Secret Sharing RadiusNeighborsRegressor (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSRadiusNeighborsRegressor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSRadiusNeighborsRegressor training in SPU")
        
        def _spu_fit(X, y, **kwargs):
            model = RadiusNeighborsRegressor(**kwargs)
            model.fit(X, y)
            return model
