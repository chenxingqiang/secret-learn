...
"""
Secret Sharing adapter for NearestCentroid

NearestCentroid is a SUPERVISED algorithm.
...
class SSNearestCentroid:
    """Secret Sharing NearestCentroid (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSNearestCentroid with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSNearestCentroid training in SPU")
        
        def _spu_fit(X, y, **kwargs):
            model = NearestCentroid(**kwargs)
            model.fit(X, y)
            return model
