...
"""
Secret Sharing adapter for RadiusNeighborsTransformer
...
try:
    from xlearn.neighbors import RadiusNeighborsTransformer
    USING_XLEARN = True
except ImportError:
...
class SSRadiusNeighborsTransformer:
    """Secret Sharing RadiusNeighborsTransformer (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSRadiusNeighborsTransformer with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSRadiusNeighborsTransformer training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = RadiusNeighborsTransformer(**kwargs)
            model.fit(X, y)
            return model
