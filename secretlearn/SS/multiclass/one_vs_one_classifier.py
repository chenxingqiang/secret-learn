...
"""
Secret Sharing adapter for OneVsOneClassifier

OneVsOneClassifier is a SUPERVISED algorithm.
...
class SSOneVsOneClassifier:
    """Secret Sharing OneVsOneClassifier (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSOneVsOneClassifier with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSOneVsOneClassifier training in SPU")
        
        def _spu_fit(X, y, **kwargs):
            model = OneVsOneClassifier(**kwargs)
            model.fit(X, y)
            return model
