...
"""
Secret Sharing adapter for OneVsRestClassifier

OneVsRestClassifier is a SUPERVISED algorithm.
...
class SSOneVsRestClassifier:
    """Secret Sharing OneVsRestClassifier (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSOneVsRestClassifier with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSOneVsRestClassifier training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = OneVsRestClassifier(**kwargs)
            model.fit(X, y)
            return model
