...
"""
Secret Sharing adapter for AdditiveChi2Sampler
...
class SSAdditiveChi2Sampler:
    """Secret Sharing AdditiveChi2Sampler (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSAdditiveChi2Sampler with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSAdditiveChi2Sampler training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = AdditiveChi2Sampler(**kwargs)
            model.fit(X, y)
            return model
