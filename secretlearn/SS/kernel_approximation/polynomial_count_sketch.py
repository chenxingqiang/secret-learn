...
"""
Secret Sharing adapter for PolynomialCountSketch
...
class SSPolynomialCountSketch:
    """Secret Sharing PolynomialCountSketch (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSPolynomialCountSketch with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSPolynomialCountSketch training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = PolynomialCountSketch(**kwargs)
            model.fit(X, y)
            return model
