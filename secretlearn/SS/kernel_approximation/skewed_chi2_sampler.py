...
"""
Secret Sharing adapter for SkewedChi2Sampler
...
class SSSkewedChi2Sampler:
    """Secret Sharing SkewedChi2Sampler (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSkewedChi2Sampler with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSkewedChi2Sampler training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = SkewedChi2Sampler(**kwargs)
            model.fit(X, y)
            return model
