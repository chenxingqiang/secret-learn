...
"""
Secret Sharing adapter for GaussianRandomProjection
...
class SSGaussianRandomProjection:
    """Secret Sharing GaussianRandomProjection (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSGaussianRandomProjection with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSGaussianRandomProjection training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = GaussianRandomProjection(**kwargs)
            model.fit(X, y)
            return model
