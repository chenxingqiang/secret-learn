...
"""
Secret Sharing adapter for SparseRandomProjection
...
class SSSparseRandomProjection:
    """Secret Sharing SparseRandomProjection (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSparseRandomProjection with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSparseRandomProjection training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = SparseRandomProjection(**kwargs)
            model.fit(X, y)
            return model
