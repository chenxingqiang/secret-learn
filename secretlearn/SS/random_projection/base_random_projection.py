...
"""
Secret Sharing adapter for BaseRandomProjection
...
try:
    from xlearn.random_projection import BaseRandomProjection
    USING_XLEARN = True
except ImportError:
...
class SSBaseRandomProjection:
    """Secret Sharing BaseRandomProjection (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSBaseRandomProjection with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSBaseRandomProjection training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = BaseRandomProjection(**kwargs)
            model.fit(X, y)
            return model
