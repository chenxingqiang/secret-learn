...
"""
Secret Sharing adapter for TheilSenRegressor
...
try:
    from xlearn.linear_model import TheilSenRegressor
    USING_XLEARN = True
except ImportError:
...
class SSTheilSenRegressor:
    """Secret Sharing TheilSenRegressor (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSTheilSenRegressor with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSTheilSenRegressor training in SPU")
