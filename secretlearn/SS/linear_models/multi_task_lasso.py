...
"""
Secret Sharing adapter for MultiTaskLasso
...
try:
    from xlearn.linear_model import MultiTaskLasso
    USING_XLEARN = True
except ImportError:
...
class SSMultiTaskLasso:
    """Secret Sharing MultiTaskLasso (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMultiTaskLasso with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSMultiTaskLasso training in SPU")
