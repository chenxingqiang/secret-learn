...
"""
Secret Sharing adapter for LassoLars
...
try:
    from xlearn.linear_model import LassoLars
    USING_XLEARN = True
except ImportError:
...
class SSLassoLars:
    """Secret Sharing LassoLars (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSLassoLars with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSLassoLars training in SPU")
