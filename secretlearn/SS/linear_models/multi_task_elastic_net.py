...
"""
Secret Sharing adapter for MultiTaskElasticNet
...
try:
    from xlearn.linear_model import MultiTaskElasticNet
    USING_XLEARN = True
except ImportError:
...
class SSMultiTaskElasticNet:
    """Secret Sharing MultiTaskElasticNet (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMultiTaskElasticNet with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSMultiTaskElasticNet training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = MultiTaskElasticNet(**kwargs)
            model.fit(X, y)
            return model
