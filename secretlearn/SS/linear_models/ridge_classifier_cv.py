...
"""
Secret Sharing adapter for RidgeClassifierCV
...
class SSRidgeClassifierCV:
    """Secret Sharing RidgeClassifierCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSRidgeClassifierCV with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSRidgeClassifierCV training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = RidgeClassifierCV(**kwargs)
            model.fit(X, y)
            return model
