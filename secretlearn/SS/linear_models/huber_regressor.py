...
"""
Secret Sharing adapter for HuberRegressor

HuberRegressor is a SUPERVISED algorithm.
...
try:
    from xlearn.linear_model import HuberRegressor
    USING_XLEARN = True
except ImportError:
...
class SSHuberRegressor:
    """Secret Sharing HuberRegressor (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSHuberRegressor with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSHuberRegressor training in SPU")
