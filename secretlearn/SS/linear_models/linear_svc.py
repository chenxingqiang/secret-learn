...
LinearSvc is a SUPERVISED algorithm.
...
try:
    from xlearn.linear_model import LinearSvc
    USING_XLEARN = True
except ImportError:
...
class SSLinearSvc:
    """Secret Sharing LinearSvc (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSLinearSvc with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSLinearSvc training in SPU")
