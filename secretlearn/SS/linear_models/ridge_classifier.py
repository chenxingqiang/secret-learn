...
"""
Secret Sharing adapter for RidgeClassifier

RidgeClassifier is a SUPERVISED algorithm.
...
class SSRidgeClassifier:
    """Secret Sharing RidgeClassifier (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSRidgeClassifier with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSRidgeClassifier training in SPU")
