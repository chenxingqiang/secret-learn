...
"""
Secret Sharing adapter for OrthogonalMatchingPursuitCV
...
try:
    from xlearn.linear_model import OrthogonalMatchingPursuitCV
    USING_XLEARN = True
except ImportError:
...
class SSOrthogonalMatchingPursuitCV:
    """Secret Sharing OrthogonalMatchingPursuitCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSOrthogonalMatchingPursuitCV with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSOrthogonalMatchingPursuitCV training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = OrthogonalMatchingPursuitCV(**kwargs)
            model.fit(X, y)
            return model
