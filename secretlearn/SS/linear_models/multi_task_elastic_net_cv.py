...
"""
Secret Sharing adapter for MultiTaskElasticNetCV
...
try:
    from xlearn.linear_model import MultiTaskElasticNetCV
    USING_XLEARN = True
except ImportError:
...
class SSMultiTaskElasticNetCV:
    """Secret Sharing MultiTaskElasticNetCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMultiTaskElasticNetCV with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSMultiTaskElasticNetCV training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = MultiTaskElasticNetCV(**kwargs)
            model.fit(X, y)
            return model
