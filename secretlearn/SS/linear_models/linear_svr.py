...
LinearSvr is a SUPERVISED algorithm.
...
try:
    from xlearn.linear_model import LinearSvr
    USING_XLEARN = True
except ImportError:
...
class SSLinearSvr:
    """Secret Sharing LinearSvr (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSLinearSvr with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSLinearSvr training in SPU")
