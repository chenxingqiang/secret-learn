...
"""
Secret Sharing adapter for Lars
...
try:
    from xlearn.linear_model import Lars
    USING_XLEARN = True
except ImportError:
...
class SSLars:
    """Secret Sharing Lars (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSLars with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSLars training in SPU")
