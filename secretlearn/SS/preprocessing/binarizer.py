...
"""
Secret Sharing adapter for Binarizer

Binarizer is an UNSUPERVISED algorithm.
...
class SSBinarizer:
    """Secret Sharing Binarizer (Unsupervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSBinarizer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSBinarizer training in SPU")
        
        def _spu_fit(X, **kwargs):
            model = Binarizer(**kwargs)
            model.fit(X)
            return model
