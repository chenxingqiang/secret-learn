...
"""
Secret Sharing adapter for PowerTransformer

PowerTransformer is an UNSUPERVISED algorithm.
...
class SSPowerTransformer:
    """Secret Sharing PowerTransformer (Unsupervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSPowerTransformer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSPowerTransformer training in SPU")
        
        def _spu_fit(X, **kwargs):
            model = PowerTransformer(**kwargs)
            model.fit(X)
            return model
