...
"""
Secret Sharing adapter for FunctionTransformer
...
class SSFunctionTransformer:
    """Secret Sharing FunctionTransformer (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSFunctionTransformer with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSFunctionTransformer training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = FunctionTransformer(**kwargs)
            model.fit(X, y)
            return model
