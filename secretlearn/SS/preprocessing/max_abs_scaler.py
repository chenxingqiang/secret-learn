...
"""
Secret Sharing adapter for MaxAbsScaler

MaxAbsScaler is an UNSUPERVISED algorithm.
...
class SSMaxAbsScaler:
    """Secret Sharing MaxAbsScaler (Unsupervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMaxAbsScaler with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSMaxAbsScaler training in SPU")
        
        def _spu_fit(X, **kwargs):
            model = MaxAbsScaler(**kwargs)
            model.fit(X)
            return model
