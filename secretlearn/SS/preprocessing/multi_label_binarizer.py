...
"""
Secret Sharing adapter for MultiLabelBinarizer
...
class SSMultiLabelBinarizer:
    """Secret Sharing MultiLabelBinarizer (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMultiLabelBinarizer with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSMultiLabelBinarizer training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = MultiLabelBinarizer(**kwargs)
            model.fit(X, y)
            return model
