...
"""
Secret Sharing adapter for Splinetransformer

Splinetransformer is a SUPERVISED algorithm.
...
class SSSplinetransformer:
    """Secret Sharing Splinetransformer (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSplinetransformer with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSplinetransformer training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = Splinetransformer(**kwargs)
            model.fit(X, y)
            return model
