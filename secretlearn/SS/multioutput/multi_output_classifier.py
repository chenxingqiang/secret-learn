...
"""
Secret Sharing adapter for MultiOutputClassifier

MultiOutputClassifier is a SUPERVISED algorithm.
...
class SSMultiOutputClassifier:
    """Secret Sharing MultiOutputClassifier (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMultiOutputClassifier with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSMultiOutputClassifier training in SPU")
        
        def _spu_fit(X, y, **kwargs):
            model = MultiOutputClassifier(**kwargs)
            model.fit(X, y)
            return model
