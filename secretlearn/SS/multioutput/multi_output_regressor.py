...
"""
Secret Sharing adapter for MultiOutputRegressor

MultiOutputRegressor is a SUPERVISED algorithm.
...
class SSMultiOutputRegressor:
    """Secret Sharing MultiOutputRegressor (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMultiOutputRegressor with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSMultiOutputRegressor training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = MultiOutputRegressor(**kwargs)
            model.fit(X, y)
            return model
