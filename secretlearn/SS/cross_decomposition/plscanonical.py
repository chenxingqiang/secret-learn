...
"""
Secret Sharing adapter for Plscanonical

Plscanonical is a SUPERVISED algorithm.
...
class SSPlscanonical:
    """Secret Sharing Plscanonical (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSPlscanonical with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSPlscanonical training in SPU")
        
        def _spu_fit(X, y, **kwargs):
            model = Plscanonical(**kwargs)
            model.fit(X, y)
            return model
