...
"""
Secret Sharing adapter for FeatureAgglomeration
...
class SSFeatureAgglomeration:
    """Secret Sharing FeatureAgglomeration (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSFeatureAgglomeration with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSFeatureAgglomeration training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = FeatureAgglomeration(**kwargs)
            model.fit(X, y)
            return model
