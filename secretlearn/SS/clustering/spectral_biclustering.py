...
"""
Secret Sharing adapter for SpectralBiclustering
...
class SSSpectralBiclustering:
    """Secret Sharing SpectralBiclustering (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSpectralBiclustering with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSpectralBiclustering training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = SpectralBiclustering(**kwargs)
            model.fit(X, y)
            return model
