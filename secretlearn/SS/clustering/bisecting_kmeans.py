...
"""
Secret Sharing adapter for BisectingKMeans
...
try:
    from xlearn.cluster import BisectingKMeans
    USING_XLEARN = True
except ImportError:
...
class SSBisectingKMeans:
    """Secret Sharing BisectingKMeans (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSBisectingKMeans with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSBisectingKMeans training in SPU")
