...
"""
Secret Sharing adapter for SpectralEmbedding

SpectralEmbedding is an UNSUPERVISED algorithm.
...
try:
    from xlearn.manifold import SpectralEmbedding
    USING_XLEARN = True
except ImportError:
...
class SSSpectralEmbedding:
    """Secret Sharing SpectralEmbedding (Unsupervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSpectralEmbedding with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSSpectralEmbedding training in SPU")
