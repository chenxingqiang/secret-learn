...
"""
Secret Sharing adapter for LocallyLinearEmbedding

LocallyLinearEmbedding is an UNSUPERVISED algorithm.
...
try:
    from xlearn.manifold import LocallyLinearEmbedding
    USING_XLEARN = True
except ImportError:
...
class SSLocallyLinearEmbedding:
    """Secret Sharing LocallyLinearEmbedding (Unsupervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSLocallyLinearEmbedding with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSLocallyLinearEmbedding training in SPU")
        
        def _spu_fit(X, **kwargs):
            model = LocallyLinearEmbedding(**kwargs)
            model.fit(X)
            return model
