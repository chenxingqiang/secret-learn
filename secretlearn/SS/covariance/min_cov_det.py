...
"""
Secret Sharing adapter for MinCovDet

MinCovDet is an UNSUPERVISED algorithm.
...
try:
    from xlearn.covariance import MinCovDet
    USING_XLEARN = True
except ImportError:
...
class SSMinCovDet:
    """Secret Sharing MinCovDet (Unsupervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMinCovDet with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSMinCovDet training in SPU")
        
        def _spu_fit(X, **kwargs):
            model = MinCovDet(**kwargs)
            model.fit(X)
            return model
