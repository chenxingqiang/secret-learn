...
"""
Secret Sharing adapter for Shrunkcovariance

Shrunkcovariance is a SUPERVISED algorithm.
...
try:
    from xlearn.covariance import Shrunkcovariance
    USING_XLEARN = True
except ImportError:
...
class SSShrunkcovariance:
    """Secret Sharing Shrunkcovariance (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSShrunkcovariance with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSShrunkcovariance training in SPU")
