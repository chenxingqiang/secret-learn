...
"""
Secret Sharing adapter for StackingClassifier
...
class SSStackingClassifier:
    """Secret Sharing StackingClassifier (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSStackingClassifier with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSStackingClassifier training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = StackingClassifier(**kwargs)
            model.fit(X, y)
            return model
