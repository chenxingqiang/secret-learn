...
"""
Secret Sharing adapter for RandomTreesEmbedding
...
class SSRandomTreesEmbedding:
    """Secret Sharing RandomTreesEmbedding (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSRandomTreesEmbedding with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSRandomTreesEmbedding training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = RandomTreesEmbedding(**kwargs)
            model.fit(X, y)
            return model
