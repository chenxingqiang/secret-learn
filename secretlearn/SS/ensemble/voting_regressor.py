...
"""
Secret Sharing adapter for VotingRegressor

VotingRegressor is a SUPERVISED algorithm.
...
class SSVotingRegressor:
    """Secret Sharing VotingRegressor (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSVotingRegressor with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSVotingRegressor training in SPU")
