...
"""
Secret Sharing adapter for SelfTrainingClassifier
...
class SSSelfTrainingClassifier:
    """Secret Sharing SelfTrainingClassifier (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSelfTrainingClassifier with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSelfTrainingClassifier training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = SelfTrainingClassifier(**kwargs)
            model.fit(X, y)
            return model
