...
"""
Secret Sharing adapter for QuadraticDiscriminantAnalysis

QuadraticDiscriminantAnalysis is a SUPERVISED algorithm.
...
class SSQuadraticDiscriminantAnalysis:
    """Secret Sharing QuadraticDiscriminantAnalysis (Supervised)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSQuadraticDiscriminantAnalysis with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSQuadraticDiscriminantAnalysis training in SPU")
