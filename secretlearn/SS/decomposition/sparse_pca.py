...
"""
Secret Sharing adapter for SparsePCA
...
try:
    from xlearn.decomposition import SparsePCA
    USING_XLEARN = True
except ImportError:
...
class SSSparsePCA:
    """Secret Sharing SparsePCA (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSparsePCA with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSparsePCA training in SPU")
