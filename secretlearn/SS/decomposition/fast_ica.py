...
"""
Secret Sharing adapter for FastIca

FastIca is a SUPERVISED algorithm.
...
try:
    from xlearn.decomposition import FastIca
    USING_XLEARN = True
except ImportError:
...
class SSFastIca:
    """Secret Sharing FastIca (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSFastIca with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSFastIca training in SPU")
