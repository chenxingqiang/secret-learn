...
"""
Secret Sharing adapter for DictionaryLearning
...
class SSDictionaryLearning:
    """Secret Sharing DictionaryLearning (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSDictionaryLearning with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSDictionaryLearning training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = DictionaryLearning(**kwargs)
            model.fit(X, y)
            return model
