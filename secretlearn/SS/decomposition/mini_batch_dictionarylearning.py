...
"""
Secret Sharing adapter for MiniBatchDictionarylearning

MiniBatchDictionarylearning is a SUPERVISED algorithm.
...
class SSMiniBatchDictionarylearning:
    """Secret Sharing MiniBatchDictionarylearning (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMiniBatchDictionarylearning with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSMiniBatchDictionarylearning training in SPU")
        
        def _spu_fit(X, y, **kwargs):
            model = MiniBatchDictionarylearning(**kwargs)
            model.fit(X, y)
            return model
