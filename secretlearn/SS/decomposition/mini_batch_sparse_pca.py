...
"""
Secret Sharing adapter for MiniBatchSparsePCA
...
try:
    from xlearn.decomposition import MiniBatchSparsePCA
    USING_XLEARN = True
except ImportError:
...
class SSMiniBatchSparsePCA:
    """Secret Sharing MiniBatchSparsePCA (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSMiniBatchSparsePCA with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSMiniBatchSparsePCA training in SPU")
