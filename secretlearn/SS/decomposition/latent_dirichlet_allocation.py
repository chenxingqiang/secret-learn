...
"""
Secret Sharing adapter for LatentDirichletAllocation
...
class SSLatentDirichletAllocation:
    """Secret Sharing LatentDirichletAllocation (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSLatentDirichletAllocation with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSLatentDirichletAllocation training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = LatentDirichletAllocation(**kwargs)
            model.fit(X, y)
            return model
