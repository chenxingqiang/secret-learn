...
"""
Secret Sharing adapter for GenericUnivariateSelect
...
try:
    from xlearn.feature_selection import GenericUnivariateSelect
    USING_XLEARN = True
except ImportError:
...
class SSGenericUnivariateSelect:
    """Secret Sharing GenericUnivariateSelect (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSGenericUnivariateSelect with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSGenericUnivariateSelect training in SPU")
        ...
        def _spu_fit(X_parts, y_parts, **kwargs):
            ...
            model = GenericUnivariateSelect(**kwargs)
            model.fit(X, y)
            return model
