...
"""
Secret Sharing adapter for SelectFdr
...
try:
    from xlearn.feature_selection import SelectFdr
    USING_XLEARN = True
except ImportError:
...
class SSSelectFdr:
    """Secret Sharing SelectFdr (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSelectFdr with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSelectFdr training in SPU")
