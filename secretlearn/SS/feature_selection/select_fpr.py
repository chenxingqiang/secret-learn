...
"""
Secret Sharing adapter for SelectFpr
...
try:
    from xlearn.feature_selection import SelectFpr
    USING_XLEARN = True
except ImportError:
...
class SSSelectFpr:
    """Secret Sharing SelectFpr (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSelectFpr with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSelectFpr training in SPU")
