...
"""
Secret Sharing adapter for SelectKBest

SelectKBest is a SUPERVISED algorithm.
...
class SSSelectKBest:
    """Secret Sharing SelectKBest (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSelectKBest with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSSelectKBest training in SPU")
        
        def _spu_fit(X, y, **kwargs):
            model = SelectKBest(**kwargs)
            model.fit(X, y)
            return model
