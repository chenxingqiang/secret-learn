...
"""
Secret Sharing adapter for SelectFwe

SelectFwe is a SUPERVISED non-iterative algorithm.
...
try:
    from xlearn.feature_selection import SelectFwe
    USING_XLEARN = True
except ImportError:
...
class SSSelectFwe:
    """Secret Sharing SelectFwe (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: 'SPU', **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSelectFwe with JAX acceleration")
    ...
    def fit(self, x: 'Union[FedNdarray, VDataFrame]', y: 'Union[FedNdarray, VDataFrame]'):
        ...
        logging.info(f"[SS] SSSelectFwe training in SPU")
