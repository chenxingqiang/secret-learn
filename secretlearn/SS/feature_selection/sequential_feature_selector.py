...
"""
Secret Sharing adapter for SequentialFeatureSelector

SequentialFeatureSelector is a SUPERVISED algorithm.
...
class SSSequentialFeatureSelector:
    """Secret Sharing SequentialFeatureSelector (Supervised)"""
    ...
    def __init__(self, spu: SPU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SS] SSSequentialFeatureSelector with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SS] SSSequentialFeatureSelector training in SPU")
        
        def _spu_fit(X, y, **kwargs):
            model = SequentialFeatureSelector(**kwargs)
            model.fit(X, y)
            return model
