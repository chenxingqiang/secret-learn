...
from secretlearn.split_learning.decomposition.nmf import SLNMF
...
# Start of _PGNMF #
###################
# This class implements a projected gradient solver for the SLNMF.
...
class _PGNMF(SLNMF):
    """Non-Negative Matrix Factorization (SLNMF) with projected gradient solver.
    ...
    def _fit_transform(self, X, y=None, W=None, H=None, update_H=True):
        ...
        check_non_negative(X, "SLNMF (input X)")
        ...
        if self.init == "custom" and update_H:
            _check_init(H, (n_components, n_features), "SLNMF (input H)")
            _check_init(W, (n_samples, n_components), "SLNMF (input W)")
        elif not update_H:
            _check_init(H, (n_components, n_features), "SLNMF (input H)")
...
def build_clfs(cd_iters, pg_iters, mu_iters):
    clfs = [
        ("Coordinate Descent", SLNMF, cd_iters, {"solver": "cd"}),
        ("Projected Gradient", _PGNMF, pg_iters, {"solver": "pg"}),
        ("Multiplicative Update", SLNMF, mu_iters, {"solver": "mu"}),
    ]
    return clfs
