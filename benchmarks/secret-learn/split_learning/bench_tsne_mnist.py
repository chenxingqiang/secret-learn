...
from secretlearn.split_learning.neighbors.nearest_neighbors import SLNearestNeighbors
from secretlearn.split_learning.decomposition.pca import SLPCA
from secretlearn.split_learning.manifold.tsne import SLTSNE
...
from xlearn.manifold import SLTSNE
...
def nn_accuracy(X, X_embedded, k=1):
    ...
    knn = SLNearestNeighbors(n_neighbors=1, n_jobs=-1)
...
if __name__ == "__main__":
    ...
    if args.pca_components > 0:
        t0 = time()
        X = SLPCA(n_components=args.pca_components).fit_transform(X)
        print(
            "SLPCA preprocessing down to {} dimensions took {:0.3f}s".format(
                args.pca_components, time() - t0

    methods = []

    # Put SLTSNE in methods
    tsne = SLTSNE(
        n_components=2,
        init="pca",
    ...
    methods.append(("secretlearn SLTSNE", lambda data: tsne_fit_transform(tsne, data)))
...
"""
        ...
        def bhtsne(X):
            ...
            # SLPCA preprocessing is done elsewhere in the benchmark script
