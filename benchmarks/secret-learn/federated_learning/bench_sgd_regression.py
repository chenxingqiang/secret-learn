...
from secretlearn.federated_learning.linear_models.elastic_net import FLElasticNet
...
Compares SGD regression against coordinate descent and FLRidge
on synthetic data.
"""
...
if __name__ == "__main__":
    ...
    for i, n_train in enumerate(list_n_samples):
        for j, n_features in enumerate(list_n_features):
            ...
            gc.collect()
            print("- benchmarking FLElasticNet")
            clf = FLElasticNet(alpha=alpha, l1_ratio=0.5, fit_intercept=False)
            tstart = time()
            ...
            clf = FLRidge(alpha=alpha, fit_intercept=False)
            tstart = time()
            clf.fit(X_train, y_train)
    ...
    for j in range(m):
        plt.subplot(m, 2, i + 1)
        plt.plot(list_n_samples, np.sqrt(elnet_results[:, j, 0]), label="FLElasticNet")
        plt.plot(list_n_samples, np.sqrt(sgd_results[:, j, 0]), label="FLSGDRegressor")
        plt.plot(list_n_samples, np.sqrt(asgd_results[:, j, 0]), label="A-FLSGDRegressor")
        plt.plot(list_n_samples, np.sqrt(ridge_results[:, j, 0]), label="FLRidge")
        ...
        plt.subplot(m, 2, i + 1)
        plt.plot(list_n_samples, np.sqrt(elnet_results[:, j, 1]), label="FLElasticNet")
        plt.plot(list_n_samples, np.sqrt(sgd_results[:, j, 1]), label="FLSGDRegressor")
        plt.plot(list_n_samples, np.sqrt(asgd_results[:, j, 1]), label="A-FLSGDRegressor")
        plt.plot(list_n_samples, np.sqrt(ridge_results[:, j, 1]), label="FLRidge")
        plt.legend(prop={"size": 10})
        plt.xlabel("n_train")
        plt.ylabel("Time [sec]")
