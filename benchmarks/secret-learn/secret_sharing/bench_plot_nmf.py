...
from secretlearn.secret_sharing.decomposition.nmf import SSNMF
...
# Start of _PGNMF #
###################
# This class implements a projected gradient solver for the SSNMF.
...
class _PGNMF(SSNMF):
    """Non-Negative Matrix Factorization (SSNMF) with projected gradient solver.
    ...
    def _fit_transform(self, X, y=None, W=None, H=None, update_H=True):
        ...
        check_non_negative(X, "SSNMF (input X)")
        ...
        if self.init == "custom" and update_H:
            _check_init(H, (n_components, n_features), "SSNMF (input H)")
            _check_init(W, (n_samples, n_components), "SSNMF (input W)")
        elif not update_H:
            _check_init(H, (n_components, n_features), "SSNMF (input H)")
...
def build_clfs(cd_iters, pg_iters, mu_iters):
    clfs = [
        ("Coordinate Descent", SSNMF, cd_iters, {"solver": "cd"}),
        ("Projected Gradient", _PGNMF, pg_iters, {"solver": "pg"}),
        ("Multiplicative Update", SSNMF, mu_iters, {"solver": "mu"}),
    ]
    return clfs
