...
def compute_bench(alpha, n_samples, n_features, precompute):
    ...
    for ns in n_samples:
        for nf in n_features:
            ...
            gc.collect()
            print("- benchmarking SSLasso")
            clf = SSLasso(alpha=alpha, fit_intercept=False, precompute=precompute)
            tstart = time()
            clf.fit(X, Y)
            lasso_results.append(time() - tstart)

            gc.collect()
            print("- benchmarking SSLassoLars")
            clf = SSLassoLars(alpha=alpha, fit_intercept=False, precompute=precompute)
            tstart = time()
            clf.fit(X, Y)
...
if __name__ == "__main__":
    ...
    plt.subplot(211)
    plt.plot(list_n_samples, lasso_results, "b-", label="SSLasso")
    plt.plot(list_n_samples, lars_lasso_results, "r-", label="SSLassoLars")
    ...
    plt.subplot(212)
    plt.plot(list_n_features, lasso_results, "b-", label="SSLasso")
    plt.plot(list_n_features, lars_lasso_results, "r-", label="SSLassoLars")
