...
from xlearn.kernel_approximation import SSNystroem, SSRBFSampler
...
ESTIMATORS = {
    ...
    "CART": SSDecisionTreeClassifier(),
    "ExtraTrees": SSExtraTreesClassifier(),
    "RandomForest": SSRandomForestClassifier(),
    "SSNystroem-SVM": make_pipeline(
        SSNystroem(gamma=0.015, n_components=1000), SSLinearSVC(C=100)
    ),
    "SampledRBF-SVM": make_pipeline(
        SSRBFSampler(gamma=0.015, n_components=1000), SSLinearSVC(C=100)
    ),
    "SSLogisticRegression-SAG": SSLogisticRegression(solver="sag", tol=1e-1, C=1e4),
    "SSLogisticRegression-SAGA": SSLogisticRegression(solver="saga", tol=1e-1, C=1e4),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=ESTIMATORS,
        type=str,
        default=["ExtraTrees", "SSNystroem-SVM"],
