...
from secretlearn.secret_sharing.manifold.tsne import SSTSNE
...
from xlearn.manifold import SSTSNE
...
def nn_accuracy(X, X_embedded, k=1):
    ...
    knn = SSNearestNeighbors(n_neighbors=1, n_jobs=-1)
...
if __name__ == "__main__":
    ...
    if args.pca_components > 0:
        t0 = time()
        X = SSPCA(n_components=args.pca_components).fit_transform(X)
        print(
            "SSPCA preprocessing down to {} dimensions took {:0.3f}s".format(
                args.pca_components, time() - t0

    methods = []

    # Put SSTSNE in methods
    tsne = SSTSNE(
        n_components=2,
        init="pca",
    ...
    methods.append(("secretlearn SSTSNE", lambda data: tsne_fit_transform(tsne, data)))
...
"""
        ...
        def bhtsne(X):
            ...
            # SSPCA preprocessing is done elsewhere in the benchmark script
