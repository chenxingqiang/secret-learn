#!/usr/bin/env python3
"""
Test ALL FL examples in background
Results saved to logs/fl_test_results.txt
"""
...
def main():
    ...
    log_file = LOG_DIR / "fl_test_results.txt"
    
    # Get all FL examples
    examples = sorted([f for f in EXAMPLES_PATH.glob("*.py") if f.name != '__init__.py'])
    total = len(examples)
    
    print(f"Testing {total} FL examples...")
    print(f"Results will be saved to: {log_file}")
    
    with open(log_file, 'w') as f:
        f.write(f"FL Examples Test Results\n")
